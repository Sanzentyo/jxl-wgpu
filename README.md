...
scalar metadata and installed directly in the resident resource table; raw Modular matrix images
remain a typed gap.
...
PQ/HLG on the GPU without an intermediate readback. `DisplayPipeline` currently converts its
narrower supported buffer set into an explicit linear-light BT.709 RGBA texture that can be sampled,
rendered, or copied to a surface without an additional host wait. Its still-unimplemented
wide-gamut/HDR input contracts are rejected instead of being mislabeled.
