...
Opt-in VarDCT stills also return validated intermediate AC images through `next_update` and its
async counterpart. DC/LF images and updates from incomplete frame input remain in progress.
...
fragmented input. `with_progressive_output(true)` now publishes complete image-wide AC-pass images
for direct color-only stills with prepared coefficient descriptors. Each immutable output validates
only completed entropy and shares one logical frame slot with its later updates. Presentation time
advances only on the final image; `next_frame` remains final-only. Whole and 256-byte windows match
native per-pass flushes within one RGB8 code and preserve exact final-only GPU bytes. DC/LF images,
deferred descriptors, extras, Modular, composition/animation and incomplete-frame readiness remain.
