...
before submission. The stock decoder now executes one Modular LF producer, converts its signed
`[Y, X, B-Y]` planes to dequantized XYB, and packs them into the dependent VarDCT LF atlas without
readback. Multi-level chains whose intermediate LF frame carries general HF-global/AC data in one
TOC packet remain unsupported.
...
implements standard Modular animation. Decoder animation, multi-level progressive-DC frames,
intermediate pass presentation, non-alpha extra channels, complete VarDCT reconstruction,
