...
| Encode | Lossless Modular Gray/GrayAlpha/RGB/RGBA with 1–31-bit integers or IEEE binary16/binary32 in packed, planar or split buffers, including RGB/BGR order, declared alpha association, enumerated or embedded RGB/Gray ICC source color and supported animation; experimental GPU VarDCT with all 27 transform strategies. | Other floating precisions, CMYK/arbitrary extra channels, YUV and texture inputs remain; VarDCT lacks general perceptual-quality guarantees, adaptive rate control, and progressive encoding. |
