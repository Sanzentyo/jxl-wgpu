...
rectangles, Replace/Add/Blend/Mul/MulAdd, unassociated Modular alpha with separate background
sources, hidden layers, reference-only frames, mixed coding modes, and recursive DC. Packing and
orientation follow composition. Gray+alpha and independent integer alpha depths also execute;
general extra-channel composition, associated alpha, pre-transform patch references, and JPEG XL
floating-point source metadata remain pending.

Modular reconstructs full-resolution integer extra channels with independent 1–16-bit precision.
...
header. Only a validated GPU ending cursor advances the color decoder. Its first unassociated
...
codes; F32 preserves signed normalization without clipping. Full-resolution integer extras now
also execute across global, LF and AC groups. Shifted/resampled and associated alpha remain pending.
...
presentation, shifted/resampled extras, associated alpha, arbitrary ICC
