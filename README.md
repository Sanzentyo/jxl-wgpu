...
fragmented input. `with_progressive_output(true)` publishes DC and complete image-wide AC-pass images
for direct color-only stills, including deferred coefficient descriptors. Each immutable output validates
...
native DC/pass flushes within one RGB8 code and preserve exact final-only GPU bytes. DC uses
image-header 8× interpolation of the complete LF atlas before ordinary restoration and output.
LF dependency images, extras, Modular, composition/animation and incomplete-frame readiness remain.
