...
`jxl-bitstream` 1.0.0 / `jxl-oxide-common` 1.0.0 interfaces. Those crates are part of jxl-oxide and
are licensed under `MIT OR Apache-2.0`; their source distributions contain the corresponding
license texts. They parse only bounded header grammar here. No jxl-oxide frame, entropy, Modular,
VarDCT, or pixel decoder is linked into this crate.

`test-data/basic.jxl.hex`, `oddsize_ups.jxl.hex`, `green_queen_vardct_e3.jxl.hex`, and
`animation_spline.jxl.hex` are byte-for-byte hexadecimal copies of the corresponding JPEG XL
fixtures in `libjxl/jxl-rs` commit `f37283edbac13f47e03e79db393438a4a2b82e07` (`jxl` 0.6.0).
