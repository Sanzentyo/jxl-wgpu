...
fn profile(@builtin(global_invocation_id) id: vec3<u32>) {
    if id.x >= metadata[7u] || id.y >= 37u { return; }
    ...
    let bins = stats + 2u + id.y * 5u * 224u;
    ...
    for (var event = 0u; event < length; event += 1u) {
        ...
        if kind == 1u && metadata[2u] == 0u {
            atomicAdd(&words[bins + context * 224u], 1u);
            ...
            if encoded.token >= 224u { atomicAdd(&words[stats + 1u], 1u); return; }
            atomicAdd(&words[bins + select(context, 0u, kind == 3u) * 224u + encoded.token], 1u);
