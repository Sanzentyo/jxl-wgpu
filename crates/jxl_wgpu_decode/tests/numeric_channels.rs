...
#[allow(dead_code)]
mod oracle;
#[path = "numeric_channels/progression.rs"]
mod progression;
    ...
    for name in [
        ...
        for channel in 0..channels {
            ...
            for (bounded, decoder) in decoders.iter().enumerate() {
                ...
                for (&word, pixel) in actual.iter().zip(color_words.chunks_exact(4)) {
    ...
    for name in [
        ...
        for channel in 0..if image.grayscale { 1 } else { 3 } {
            ...
            for (bounded, decoder) in decoders.iter().enumerate() {
                ...
                let mut max = 0_f32;
                for (&actual, pixel) in actual.iter().zip(color.chunks_exact(4)) {
