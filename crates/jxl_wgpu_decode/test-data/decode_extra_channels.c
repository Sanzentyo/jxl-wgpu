 ...
 * Prints little-endian F32 RGBA followed by every full-resolution extra-channel plane.
 * cc decode_extra_channels.c $(pkg-config --cflags --libs libjxl) -o /tmp/jxl-extra-oracle
 * /tmp/jxl-extra-oracle INPUT.jxl > CHANNELS.f32
 */
...
int main(int argc, char** argv) {
  ...
  if (!dec || JxlDecoderSubscribeEvents(dec, JXL_DEC_BASIC_INFO | JXL_DEC_FULL_IMAGE) != JXL_DEC_SUCCESS
  ...
  for (;;) {
    ...
    if (status == JXL_DEC_BASIC_INFO) {
      ...
      for (uint32_t c=0; c<info.num_extra_channels; ++c) {
        ...
        plane_size = size; extras[c] = malloc(size); if (!extras[c]) return 2;
