...
fn raw_matrix_late_admission_shrinks_uploads_and_cancellation_releases_every_stage() {
    ...
    for (local_packets, cancel_stage) in [false, true]
        .into_iter()
        .flat_map(|local| (0..3).map(move |stage| (local, stage)))
    {
        let mut session = session(&backend, &pipelines, 0, local_packets);
        ...
        let (weak_image, weak_frame) = loop {
            ...
            let mapping = pending.stage_completion().wait();
            pending.advance_staged_packet(mapping).unwrap();
        };
...
fn raw_matrix_rejects_an_unaffordable_minimum_without_retaining_frame_memory() {
    ...
    for local_packets in [false, true] {
        let mut session = session(&backend, &pipelines, 4, local_packets);
