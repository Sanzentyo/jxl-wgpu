...
/// Equal color/extra resampling is required when patches precede frame upsampling.
/// Keep a full LF chain and its independently decodable root for each source codec.
pub(super) fn generate_patch_seeds(temporary: &Path, output: &Path, decoder: &Path) {
    ...
    for row in manifest.lines() {
        ...
        for mode in ["modular", "vardct"] {
            let seeds: Vec<_> = [("vardct", 0), (mode, 1)]
                .into_iter()
                .map(|(mode, level)| {
                    let data =
                ...
                .collect();
            for (suffix, lowest) in [("", 0), (".lf1", 1)] {
                let encoded = chain(&seeds, lowest);
                validate(decoder, temporary, &encoded);
                std::fs::write(
                    output.join(format!("{name}_{mode}{suffix}.jxl.hex")),
                    hex::hex(&encoded),
                )
                .unwrap();
            }
