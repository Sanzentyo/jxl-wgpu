...
use jxl_gpu_protocol::{ColorMatrix, DisplayIntensity, RgbColorSpace, WhitePointAdaptation};
...
pub fn display_luminance(
    ...
    let intensity = DisplayIntensity::new(intensity_target).ok_or_else(|| {
        Error::InvalidPayload("display intensity must be finite and positive".into())
    })?;
    ...
    let gamma = 1.2 * 1.111_f64.powf((f64::from(intensity.nits()) / 1000.0).log2());
