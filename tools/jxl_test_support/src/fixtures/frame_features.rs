//! Explicit frame-feature assembly with independently written ANS prefixes and native image entropy.
...
pub(super) fn flags(writer: &mut BitWriter, value: u64) {
    match value {
...
pub(super) fn feature_header(bytes: &[u8], frame: &FrameInventory, new_flags: u64) -> BitFragment {
...
pub(super) fn header(
    ...
    writer
        ...
        .unwrap();
    flags(&mut writer, source.flags | feature_flags);
...
pub(super) fn assemble_frame_sequence(frames: &[Frame<'_>], repeat_lf: bool) -> Vec<u8> {
    ...
    for (index, input) in frames.iter().enumerate() {
        ...
        for values in [input.patches, input.splines].into_iter().flatten() {
            let entropy = dictionary(values);
            copy_bits(&mut prefix, entropy.as_bytes(), 0, entropy.bit_len() as u64);
        }
        let feature_flags =
            u64::from(input.patches.is_some()) * 2 + u64::from(input.splines.is_some()) * 16;
        result.extend(packet_frame_prefix(
            bytes,
            frame,
            header(&image, frame, input.reference, feature_flags),
            (prefix.bit_len() != 0).then_some(&prefix),
        ));
    }
    result
}

pub(super) fn packet_frame_prefix(
    bytes: &[u8],
