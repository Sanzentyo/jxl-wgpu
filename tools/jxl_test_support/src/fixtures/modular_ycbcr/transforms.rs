...
impl NativeTopology {
    pub fn parse(text: &str) -> Self {
        let words: Vec<i64> = text
            .split_whitespace()
            .map(|word| word.parse().unwrap())
            .collect();
        let (header, channels) = words.split_at(4);
        let count = usize::try_from(header[1]).unwrap();
        assert_eq!(channels.len(), count * 4);
        Self {
            meta_channels: usize::try_from(header[0]).unwrap(),
            transform_count: usize::try_from(header[2]).unwrap(),
            squeeze_channels: usize::try_from(header[3]).unwrap(),
            channels: channels.as_chunks::<4>().0.to_vec(),
        }
    }
}

fn split(horizontal: bool, in_place: bool, begin_channel: u32, channel_count: u32) -> Squeeze {
    Squeeze {
        horizontal,
        in_place,
        begin_channel,
...
fn rct(rct_type: u32, begin_channel: u32) -> Transform {
    Transform::Rct {
        begin_channel,
        rct_type,
    }
...
fn palette(begin_channel: u32, channel_count: u32) -> Transform {
    Transform::Palette {
        begin_channel,
        channel_count,
    }
...
pub(super) fn extend(cases: &mut Vec<Case>) {
    for kind in 0..42 {
        ...
        case.selectors = [0; 3];
        case.transforms = vec![rct(kind, 0)];
        cases.push(case);
    }
    for cb in 0..4 {
        for y in 0..4 {
            for cr in 0..4 {
                ...
                case.transforms = vec![default_squeeze()];
                cases.push(case);
            }
        }
    }
    ...
    for in_place in [false, true] {
        ...
        case.transforms = vec![Transform::Squeeze(vec![
    ...
    lf.passes = 2;
    lf.transforms = vec![Transform::Squeeze(
        (0..3)
    ...
    passes.passes = 2;
    passes.transforms = vec![default_squeeze()];
    cases.push(passes);
    for bits in [8, 31] {
    ...
    for channel in 0..3 {
        for grouped in [false, true] {
            ...
            case.transforms = vec![palette(channel, 1)];
            cases.push(case);
        }
    }
    let mut case = Case::new("palette_rgb".into());
    case.selectors = [0; 3];
    case.transforms = vec![palette(0, 3)];
    cases.push(case.clone());
    case.name = "rct_palette_squeeze".into();
    case.transforms = vec![rct(6, 0), palette(0, 3), default_squeeze()];
    cases.push(case.clone());
    case.name = "palette_squeeze".into();
    case.selectors = [1, 2, 3];
    case.transforms = vec![palette(0, 1), default_squeeze()];
    cases.push(case);
    let mut residual = Case::new("squeeze_residual_rct".into());
    residual.selectors = [0; 3];
    residual.transforms = vec![
    ...
    extras.transforms = vec![Transform::Squeeze(vec![
    ...
    restored.transforms = vec![palette(0, 1), default_squeeze()];
    cases.push(restored);
}
