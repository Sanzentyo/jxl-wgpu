...
| `VDCT-D02` | P0 | **Partial** | Sectioned packets now GPU-decode mixed strategy maps for all 27 regular and special strategies, capacity-strided quant fields, `hf_mul`, extra precision, per-frequency-cell HF chroma correlation, and arbitrary valid block-context maps selected from stream-defined quant-field and signed X/Y/B LF thresholds. An actual-GPU scalar context matrix and a mixed libjxl image cover the production path without lowering a legal strategy to DCT8. Completion requires multiple LF groups, required Modular side images, broader mixed-strategy/correlation conformance coverage, and cross-group addressing. | `VDCT-D01`, `MOD-D05` |
...
| `VDCT-D04` | P0 | **Partial** | The stock decoder now dispatches every one of the 27 compact strategy buckets through the resident regular or special inverse-transform kernels, sharing one task artifact and scratch plan. Kernel tests cover every strategy and a libjxl mixed map covers odd padded edges through the full decoder. Completion requires libjxl coverage of every strategy in mixed maps, multiple LF groups, and ISO 18181-3 precision evidence. | `VDCT-D02`, `VDCT-D03` |
| `VDCT-D05` | P0 | **Partial** | Every normative default strategy matrix, stream-selected global/LF/HF scales, extra precision, quant bias, adaptive LF smoothing, DC prediction, and per-cell HF chroma correlation execute on GPU for the bounded one-LF-group profile. Matrix orientation is explicit for regular, wide, and special coefficient layouts. Completion requires custom matrices, non-default LF channel-correlation metadata, Modular side images, and cross-LF-group behavior. | `VDCT-D02`, `VDCT-D03` |
