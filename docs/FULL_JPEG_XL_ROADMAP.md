...
Status date: 2026-09-03. This document is the canonical capability and implementation backlog for
...
| Restoration/render graph | **Partial** | Reusable upsampling, Gaborish, EPF, blend, color, and display kernels exist in `jxl_wgpu`; 3-plane and single-plane 1×/2×/4×/8× upsampling primitives exist; the bounded stock VarDCT decoder expands only shifted JPEG components before restoration, constructs one full-image sigma plane, and routes Gaborish plus signaled EPF0/EPF1/EPF2 across LF-group boundaries through one resident ping-pong scratch set in the same submission. Single-plane upsampling primitive is implemented while E2E Extra Channel decode/scheduler/alpha connection and composition remain in progress. |
| Output formats | **Partial** | Native integer Gray/RGB/RGBA and 30 portable VPI pitch-linear outputs exist for the lossless Gray8 conversion path; VarDCT currently returns tightly packed RGB8, BGR8, and opaque RGBA8/BGRA8 (with alpha 255). |
...
| `VDCT-D07` | P1 | **Partial** | The public VarDCT decoder executes JPEG reconstruction's 4:4:4, 4:2:2, 4:4:0, and 4:2:0 component layouts with normative quarter/three-quarter weights and replicated odd borders inside the resident output pass. Subsampled streams with signaled adaptive LF smoothing are rejected under `Strict` policy with `UnsupportedSubsampledStage` and safely bypassed under default `ExplicitCompatibilityFallback` policy with `AdaptiveLfDisposition::BypassedSubsampled` recorded in session memory stats, routing directly to component LF offsets to prevent coordinate mismatch. The same horizontal/vertical kernels, with a fused two-axis form, expand shifted components before a signaled full-resolution restoration sequence without readback. Completion requires a valid subsampled-restoration interoperability fixture, uncommon asymmetric JPEG component layouts, and group halos. | `MOD-D05`, `RENDER-01` |
...
| `IO-01` | P1 | **Partial** | The generic resident/readback session supports native RGB/BGR/RGBA and every classified pitch-linear color layout, including NV12-family, packed 4:2:2, P010/P012/P016, odd extents, range, chroma siting, BT.601/709/2020 NCL/2020 CL matrices, wide-gamut primaries, and SDR/HDR transfer conversion. The public VarDCT decoder routes native packed RGB8, BGR8, and opaque RGBA8/BGRA8 (alpha 255) outputs directly on GPU without CPU pixel conversion. Completion requires routing remaining pitch-linear color contracts from VarDCT plus native Gray/alpha/extra-channel combinations. | `COLOR-01`, both decoders |
