...
and `QA-03/06` remain **Partial**; independently declared extra precisions/types, embedded ICC,
...
| `FRAME-05` | P1 | **Partial** | Modular GrayAlpha/RGBA animations now preserve declared source alpha association through all five blend modes, independent color/alpha references and signed crops, with native/Rust composed output and GPU timing evidence. Extend to VarDCT/mixed frames, arbitrary extra-channel blends, hidden/reference frames, names, previews, and frame indexing. | encoder mode items, `CONT-03` |
