...
| `FRAME-05` | P1 | **Partial** | Modular GrayAlpha/RGBA animations now preserve declared source alpha association through all five blend modes, independent color/alpha references and signed crops, with native/Rust composed output and GPU timing evidence. RGB/Gray embedded-ICC Replace animations additionally bind exact profile identity and retain one budgeted stream header across changing source layouts. VarDCT now emits RGB8/XYB animations through single/mixed-transform and tiled backends, retaining AC passes, signed crops, Replace/Add/Multiply, hidden regular frames and all four post-color-transform slots. Both codecs share a checked immutable frame-control plan before GPU admission, including all Modular scheduling paths. Eight sequences cover independent native/jxl-oxide composition, exact timing, whole/bounded GPU output, retained leases, admission retry and cancellation. Both animation sessions emit bounded plain `jxli` through common assembled-header planning; indexed targets preserve exact GPU bytes, including hidden frames and older-reference restart across a newer independent anchor. Both codecs now emit reference-only animation frames in all four slots; the common control plan governs absent fields and implicit single-pass VarDCT dispatch, packets and admission. Seven additional sequences cover scalar native original Modular words, independent native/jxl-oxide composition, both entropy completion paths, whole/bounded GPU output and indexed seeking. Exact-budget rejection and cancellation retain ownership. See the [animation corpus](CONFORMANCE_CORPUS.md#vardct-animation-encoding) and [reference-only evidence](CONFORMANCE_CORPUS.md#reference-only-frame-encoding). Mixed coding modes, layered stills, VarDCT pre-transform references, arbitrary extra-channel blends, names, previews and broader indexing policy remain. | encoder mode items, `CONT-03` |
