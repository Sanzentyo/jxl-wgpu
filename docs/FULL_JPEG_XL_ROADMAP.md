...
| Restoration/render graph | **Partial** | Reusable upsampling, Gaborish, EPF, blend, color, and display kernels exist in `jxl_wgpu`; planning supports identity/no-op and 2×/4×/8× resampling with GPU interpolation kernels executing only 2×, 4×, and 8× factors; the bounded stock VarDCT decoder expands only shifted JPEG components before restoration, constructs one full-image sigma plane, and routes Gaborish plus signaled EPF0/EPF1/EPF2 across LF-group boundaries through one resident ping-pong scratch set in the same submission. Single-plane upsampling primitive is implemented while E2E Extra Channel decode/scheduler/alpha connection and composition remain in progress. |
