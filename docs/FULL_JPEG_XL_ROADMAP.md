...
| Lossless Modular encode | **Partial** | Gray/RGB/RGBA integer input, 1–31 bits in native U8/U16/U32 storage, 256×256 groups, one pass, fixed Gradient/YCoCg and prefix+RLE/LZ77 profile; standard animation is implemented. Exact original-integer, native normalized and whole/bounded GPU evidence covers every high depth and selected full-Replace animations. |
...
those items remain **Partial**; floating input, other layouts, independent extras, adaptive
...
| `IO-02` | P1 | **Partial** | Packed native U8/U16/U32 Gray/RGB/RGBA buffers now encode every 1–31-bit integer depth, including unaligned row padding and ignored high padding bits. Broader portable RGB/BGR/YUV buffers and textures remain; normalize planar/semi-planar formats and bit alignment in WGSL with no host pixel conversion. | `COLOR-01`, encoder modes |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `ENC-01` | P1 | **Partial** | Lossless Modular accepts all 1–31-bit packed integer Gray/RGB/RGBA sources with exact original-word conformance and unassociated alpha. Floating-point precision, other alpha association, arbitrary extra-channel planes, color/ICC metadata, planar buffers and textures remain; all input normalization must stay on GPU. | `IO-02`, `COLOR-03` |
...
| `QA-03` | P0 | **Partial** | Differentially compare decode with libjxl and encode with both libjxl/`djxl` and independent Rust oracles (`jxl`, or retained jxl-oxide integer planes for exact high-depth words). Cover cross-products of tools, not only isolated single-feature fixtures. | each feature |
