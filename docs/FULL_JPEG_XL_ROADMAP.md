...
| `FRONT-03` | P1 | **Partial** | Transport accepts arbitrary owned chunks without collecting a whole stream: apart from the inline signature, ordered raw/`jxlc`/v0-`jxlp` payloads share caller `Arc` storage and gap-blocked v1 fragments use a separately limited retained buffer. `CodestreamStreamScanner` then reconstructs only bounded image/frame-header/TOC probes, emits `Arc` frame inventories before section data, and routes physical section ranges in logical codestream order. Both decoders and the internal coding-mode selector accept a checked logical shared-span source. Modular and VarDCT metadata parsing is span-native; VarDCT includes MA descriptors, HF block-context maps, custom coefficient-order permutations, and cursor-dependent local-HF headers. All LF/combined/HF/AC uploads use exact span ranges, and the temporary whole-stream GPU buffer is initialized directly from spans without a second host-sized `Vec`. Byte-drip animation, entropy-permuted TOCs, v1 reorder, truncation, bounded prefix retention, shared section backing, span gaps/overlaps, every Modular fixture split, every split of a 257x17 VarDCT fixture, one-byte-span custom orders/HF continuation, and a three-span VarDCT upload are covered. Completion still requires connecting section events to the engines, bounding source-span lifetime with GPU backpressure/cancellation, and removing the contiguous `GpuDecoder::open` prerequisite. | `CONT-01` |
...
all VarDCT scalar metadata, scheduling, and uploads now share the chunk-capable source contract
internally. The immediate P0 gap remains the
common frontend and entropy work in `FRONT-01` and `ENT-D01/02`: connect section events to both
engine boundaries, bound retained span lifetime, and add recursive-stream resume. In
