...
| `ENT-D02` | P0 | **Partial** | Every accepted stock Modular MA profile now splits an oversized group into ordered GPU submissions over one reusable upload. Adjacent segments carry 16-byte backward/forward overlap and yield only between complete output tokens. A 16-byte-aligned per-lane POD tail preserves the common logical bit cursor, ANS state, LZ77 copy cursor/count/history position, decoded counts, last value, and first failure in 32 bytes; generic MA adds gradient history for 48 bytes, while Weighted/SelfCorrecting MA persists four true errors plus twelve subprediction-error accumulators in 112 bytes. Channel boundaries reset predictor-local state. The stream cap is caller-configurable and further bounded by the device and shared per-frame byte budget. Host scheduling tests cover unaligned starts, overlap mapping, lane isolation, budget adaptation, undersized caps, and segmentation of every oversized accepted profile. Actual-adapter fixtures force both a 193×97 Prefix+RLE/LZ77 fixed-Gradient stream and a checked-in libjxl 0.12.0 193×197 ANS Weighted stream through a 256-byte cap; blocking and runtime-neutral async output must equal source/Rust-`jxl` oracles, abandonment must release its reservation, and late-window destructive corruption must return typed group-specific GPU entropy failure from the final aggregate map. `WgpuDecodeMemoryStats` exposes the parsed Prefix/ANS representation and exact selected state size. Completion still requires bounded resume for VarDCT LF/HF consumers and recursive streams plus broader corruption/truncation fuzz coverage. | `ENT-D01` |
