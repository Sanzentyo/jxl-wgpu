...
| VarDCT encode | **Partial** | All 27 strategy identifiers execute in fixed-transform form. Bounded and image-wide tiled DCT8 perform real default-matrix AC quantization, natural-order tokenization, and prefix packing on GPU. Every coefficient context maps to one distribution with LZ77 disabled. Tiled DCT8 includes standard fused single packets, partial edges, multiple AC/LF groups and checked axes through 16K; independent f64 coefficient and Rust/libjxl/GPU image tests cover group boundaries. The other 26 strategies remain LF-only; mixed maps, general distance/quality guarantees, rate control and progressive encoding are absent. |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `VDCT-E01` | P0 | **Partial** | All 27 fixed-transform identifiers and arbitrary-extent tiled DCT8 execute. Validated exact-binary16 LF dequantization and LF/HF chroma-correlation metadata share the serializer and quantizers. Tiled DCT8 handles nonzero dimensions through the checked 16K axis bound, including tiny/odd single-packet images and every 2048-pixel LF/256-pixel AC group. GPU edge replication, per-LF Gradient prediction and two-dimensional dispatch preserve orientation. Actual-GPU coefficient checks include a nonzero-AC 2057x2057 four-LF-group image; Rust `jxl`, `djxl` and stock GPU presentation agree within one RGB8 code for the explicit single/multi-group procedural cases. Exact-black 16384x1 and 1x16384 exercise eight LF groups and 64 AC groups. Completion still requires mixed strategy maps, non-DCT8 edge transforms, every-strategy forward/inverse scalar oracles, and device-bounded image-wide execution beyond the exercised axis profile. | `VDCT-D04` |
| `VDCT-E02` | P0 | **Partial** | Bounded and tiled DCT8 quantize real AC with default matrices, natural order and one prefix cluster for all 495 contexts, with LZ77 disabled. Tiled coefficients stay in 2 KiB workgroup memory; each block owns a word-aligned bit fragment, checked before host concatenation in AC-group raster order. A 512-byte parameter and exact artifact/readback reservation cover fragment bounds and completion lifetime. Independent f64 cosine-sum/quantizer checks cover eight boundary geometries, including 2057x2057 and custom correlation, within one integer quantizer step; eleven fused/sectioned image cases agree across Rust `jxl`, `djxl` and the stock GPU decoder within one RGB8 code. All five linear workgroup variants are byte-identical for nine configurations. Malformed/missing artifacts, device limits, exact/one-byte-deficient budgets and abandoned nonzero-job reuse are tested. Completion requires non-DCT8 strategy/order support, adaptive clustering or ANS/LZ policy, multiple passes, broader quantizer/coefficient conformance, and bounded execution for larger images. Contextual/stateful policies must preserve entropy state on GPU instead of reusing independently concatenated prefix blocks. | `ENT-E01`, `VDCT-E01`, `VDCT-D03` |
...
| `QA-02` | P0 | **Partial** | The official animation_spline input and unchanged content-addressed F32 reference now validate all 60 frames against their individual RMSE/peak limits, plus exact whole/bounded-window agreement and timing metadata. The test verifies reference SHA-256 and shape before comparison. Completion requires the remainder of the ISO/IEC 18181-3 suite or an auditable equivalent with per-test precision results. This one official case and local round trips cannot produce a “full decoder” claim. | full decode path |
...
Linear/sRGB/BT.709/Gamma/DCI transfer; non-D65 original metadata currently requires relative intent.
GPU output offers explicit Bradford or absolute-XYZ conversion; display accepts the new profiles.
The shared image/display uniforms are 208/160 bytes. Original reconstruction and general CMS
...
and LUT profiles, CMYK, original rendering intents, HDR luminance/OOTF, wide-gamut feature/LF
