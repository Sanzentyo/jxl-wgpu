...
| `ENT-E01` | P1 | **Partial** | Modular now has explicit greedy GPU LZ77 search over arbitrary residual sequences, overlapping and regular-distance matches, GPU literal/length/distance histograms, checked artifacts and canonical prefix metadata. A 32-candidate hash chain has bounded per-group/channel storage and preserves Weighted state. Independent entropy decoding covers distances through 2²⁰; native/original-word and whole/bounded GPU matrices cover all predictors, integer/IEEE words, RCT, group sizes, animation and streamed cancellation. Default Prefix/zero-run bytes remain stable. Explicit ANS now serializes complete groups on GPU with deterministic frame-wide 4096-slot distributions, shared state across transformed channels, bounded output and the existing two-pass batch lease. Independent exhaustive alias/histogram and GPU token/state/cursor checks complement every-depth/predictor/RCT, Palette/program, animation, malformed-state, exact-budget, cancellation and late-overflow evidence; see [ANS coverage](CONFORMANCE_CORPUS.md#lossless-modular-gpu-ans-encoding). The immutable ANS codebook now shares one to five distributions across its four channel contexts and distance, searching all 52 partitions with deterministic fixed-point rate estimates and exact repeated-header costs. Independent exhaustive context-map/rate checks and 208 GPU dispatches cover every partition, reversed labels, both LZ77 modes and exact cursor/state termination; public one/four-cluster cases retain native/GPU word, layout and ownership evidence. Each union now jointly selects among all 37 full-u32-safe residual/distance hybrid configurations below LZ77 symbol 224, using GPU profiles, exact extra-bit costs and one immutable configuration for wire/GPU lowering. All 37 settings under both LZ77 modes independently decode exact GPU words/cursors; joint-search rate, corrupted-profile, binding-limit and public native/ownership checks cover the added bounded arena. The same joint search now selects the global LZ77 length split from all five full-20-bit settings fitting its 32 symbols, with exact extra-bit/repeated-header costs and bounded canonical-histogram conversion. Complete-domain rebinning, all 370 length/raw/LZ combinations, independent joint-rate checks and native/GPU short-match headers cover selection without additional GPU profiling. Learned/wider contexts, clustering outside Modular ANS, adaptive alphabet choices, cross-channel search and broader effort policy remain. `djxl`/`jxl` must accept every generated family. | — |
...
| `IO-02` | P1 | **Partial** | Gray/GrayAlpha/RGB/RGBA buffers now accept packed, planar and split components with bijective RGB/BGR/alpha swizzles, arbitrary integer bit positions, 8/16/24/32-bit storage words and Native/Little/Big byte order. Every 1–31-bit integer depth and IEEE binary16/binary32 retain exact source words. Independent unaligned offsets/pitches are validated and rebound per plane; source and artifact limits both split GPU batches. Independent exact/native references, canonical byte equality, whole/bounded GPU output, mixed-layout animations and admission/cancellation tests cover the contract. Enumerated full-range RGB/Gray metadata retains standard/custom primaries/white, seven transfers, four intents and explicit image white. Source alpha association is declared without changing samples, including invisible colors, and final alpha presentation has independent native evidence. Embedded RGB/Gray ICC bytes and original samples now survive both ordinary and explicit device-channel layouts with typed limits, independent/native comparisons and budgeted header ownership. Other floating precisions, CMYK/arbitrary extras, YUV and textures remain; image-domain normalization stays on GPU. | `COLOR-01`, encoder modes |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `ENC-01` | P1 | **Partial** | Lossless Modular accepts all 1–31-bit integer and IEEE binary16/binary32 Gray/GrayAlpha/RGB/RGBA sources with exact original-word conformance and declared associated/unassociated alpha. Packed/planar/split layouts, swizzles, word byte order and integer bit positions are read directly on GPU. Floating samples retain special values and payloads through shared/local trees, animations with per-frame layout changes and bounded streaming. Enumerated RGB/Gray source color, four intents and explicit image white survive still and Replace animation serialization with independent native profile/pixel checks. Alpha composition retains independent reference fields and final output-policy semantics; zero-alpha source colors remain untouched. Embedded RGB/Gray ICC now preserves every original byte with explicit device-channel input, animation identity, size limits and independently verified pixel/profile output. Other floating precisions, CMYK/arbitrary extra-channel planes, YUV and textures remain; all input normalization must stay on GPU. | `IO-02`, `COLOR-03` |
