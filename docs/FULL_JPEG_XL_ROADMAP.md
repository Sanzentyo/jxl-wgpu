...
| `ENT-E01` | P1 | **Partial** | Modular now has explicit greedy GPU LZ77 search over arbitrary residual sequences, overlapping and regular-distance matches, GPU literal/length/distance histograms, checked artifacts and canonical prefix metadata. A 32-candidate hash chain has bounded per-group/channel storage and preserves Weighted state. Independent entropy decoding covers distances through 2²⁰; native/original-word and whole/bounded GPU matrices cover all predictors, integer/IEEE words, RCT, group sizes, animation and streamed cancellation. Default zero-run bytes remain stable. GPU ANS serialization, histogram/context clustering, hybrid-uint selection, cross-channel search and broader effort policy remain. `djxl`/`jxl` must accept every generated family. | — |
