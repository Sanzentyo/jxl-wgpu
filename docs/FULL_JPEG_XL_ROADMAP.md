...
|---|---|---|
| Raw/`jxlc`/`jxlp` transport and header inventory | **Partial** | Bounded still-image transport, frame/TOC inventory, ICC reconstruction, and feature metadata exist. A non-accumulating scanner emits arbitrary-chunk raw/`jxlc`, ordered-v0, and bounded out-of-order-v1 `jxlp` payloads plus byte-preserving auxiliary-box events; incremental codestream inventory and decode integration remain incomplete. |
...
| `FRONT-03` | P1 | **Partial** | The transport layer accepts arbitrary owned chunks and emits logically ordered codestream ranges without collecting a whole stream: apart from the inline reconstructed two-byte signature, ordered raw/`jxlc`/v0-`jxlp` payloads share caller `Arc` storage, while future v1 fragments coalesce arbitrary chunks into one retained payload buffer per fragment under a separate logical-byte limit. Completion still requires resumable image/frame-header and TOC parsing, direct section/window delivery into both GPU engines, arbitrary splits around TOCs/entropy words, bounded end-to-end cancellation, and removal of the contiguous `GpuDecoder::open` prerequisite. | `CONT-01` |
| `CONT-01` | P1 | **Partial** | The contiguous parser and incremental `ContainerStreamScanner` validate naked streams, `jxlc`, delivery-order v0 and indexed/out-of-order v1 `jxlp`, compact/extended/to-end box sizes, fragment order/completeness, typed input/box/codestream/buffer limits, and end-of-input. Auxiliary start/chunk/end events preserve exact 8/16-byte headers and shared payload ranges; only gap-blocked future fragments are copied and their live/peak bytes are observable. Every two-chunk split, byte-drip transport, version-1 reorder, truncation-prefix equivalence, zero-copy identity, and poisoned-state tests are checked in. Completion requires the full current-version ordering/compatibility rules, explicit requested-box retention policy, and integration with the incremental codestream/frontend API. | — |
...
multi-LF-group tiled-DCT8 LF-only encoder are implemented. Incremental transport now reaches a
bounded byte-preserving event stream without whole-input assembly, but inventory and GPU consumers
remain contiguous. The immediate P0 gap remains the common frontend and entropy work in
`FRONT-01` and `ENT-D01/02`: connect resumable header/TOC/section state to the existing windows and
add recursive-stream resume. In parallel, `VDCT-E02` must emit real
