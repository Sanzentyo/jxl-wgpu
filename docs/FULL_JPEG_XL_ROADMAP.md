...
| `MOD-D03` | P0 | **Partial** | The production metadata path parses arbitrary ordered stacks with all 42 validated RCT types, Palette fields including delta storage and predictors, and explicit/default horizontal/vertical Squeeze. A bounded typed IR meta-applies exact channel order, odd average/residual geometry, shifts, bit depth, and meta-channel boundaries; its 32-byte `Pod` geometry/layout records have compile-time ABI assertions and checked WGSL-u32 offsets. Reverse traversal reconstructs the prior topology one elementary operation at a time with only two live channel vectors, validates Palette/meta and Squeeze average/residual pairing, and is protected by a cumulative topology-work limit. A standalone resident Squeeze pipeline now executes horizontal/vertical odd-tail reconstruction from three non-overlapping views of one read-write arena. Its 64-byte `Pod` uniform and portable two-word arithmetic preserve the normative `i64` smooth tendency and wrapping `i32` samples; Naga validation and an actual-adapter scalar oracle cover extreme and one-dimensional cases without shader-source string tests. An optional real `cjxl --progressive_dc=2` fixture fixes the LF2 root as one default-Squeeze transform with 13 parameters, 40 entropy channels, leading 8×8/4×4/4×4 geometry, and conserved sample count. Completion requires connecting lifetime allocation and reverse scheduling, resident delta/lossy Palette and arbitrary-RCT pixels, arbitrary composition order on actual GPU, and libjxl pixel conformance. | `MOD-D01` |
