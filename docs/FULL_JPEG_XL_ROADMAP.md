...
| `FRONT-03` | P1 | **Partial** | Transport accepts arbitrary owned chunks without collecting a whole stream: apart from the inline signature, ordered raw/`jxlc`/v0-`jxlp` payloads share caller `Arc` storage and gap-blocked v1 fragments use a separately limited retained buffer. `CodestreamStreamScanner` then reconstructs only bounded image/frame-header/TOC probes, emits `Arc` frame inventories before section data, and routes physical section ranges in logical codestream order. Both decoders retain a checked logical shared-span source and copy bounded upload ranges across physical chunks. Modular metadata parsing is span-native. VarDCT scheduling uses only logical length, all LF/combined/HF/AC window uploads use exact span ranges, and the temporary whole-stream GPU buffer is initialized directly from spans without a second host-sized `Vec`; its initial and cursor-dependent metadata parsers remain single-slice. Byte-drip animation, entropy-permuted TOCs, v1 reorder, truncation, bounded prefix retention, shared section backing, span gaps/overlaps, every Modular fixture split, and a three-span VarDCT segment are covered. Completion still requires connecting section events to the engines, converting the remaining VarDCT metadata readers, bounding source-span lifetime with GPU backpressure/cancellation, and removing the contiguous `GpuDecoder::open` prerequisite. | `CONT-01` |
...
multi-LF-group tiled-DCT8 LF-only encoder are implemented. Incremental transport and bounded
image/frame/TOC inventory now route physical section events without whole-input assembly, but both
GPU consumers still enter through contiguous public ownership. Modular metadata plus uploads and
VarDCT scheduling plus uploads now share the chunk-capable source contract internally; VarDCT's
initial and cursor-dependent metadata parsers remain contiguous. The immediate P0 gap remains the
common frontend and entropy work in `FRONT-01` and `ENT-D01/02`: connect section events to both
engine boundaries, finish VarDCT metadata conversion, bound retained span lifetime, and add recursive-stream resume. In
