...
| `ENT-D02` | P0 | **Partial** | Every accepted stock Modular MA profile and the VarDCT AC pass-group consumer split oversized entropy into ordered GPU work over one reusable upload. Adjacent segments carry 16-byte backward/forward overlap and yield only between complete output tokens. Modular uses 32/48/112-byte aligned resume records. VarDCT AC uses a 464-byte `Pod` record holding common ANS/LZ state, nested block/channel/coefficient progress, sticky sink failure, and the three-channel 96-word nonzero-neighbour grid. Channel boundaries reset Modular predictor-local state; only a final segment performs exact ANS/padding termination. The shared stream cap is caller-configurable and bounded by device limits; Modular additionally adapts it against the per-frame budget. Host scheduling tests cover unaligned starts, overlap mapping, lane isolation, budget adaptation, undersized caps, and segmentation of every oversized Modular profile. Actual-adapter fixtures force 193×97 Prefix+RLE/LZ77 Gradient, libjxl 193×197 ANS Weighted, and libjxl 438×589 nonzero/custom-order VarDCT AC through 256-byte caps. Blocking and runtime-neutral async outputs match their source/Rust-`jxl` oracles, abandonment releases reservations, and late-window damage returns typed group-specific GPU entropy failure from the final aggregate map. Memory stats expose the selected state/window/batch sizes. Completion still requires bounded resume for VarDCT LF/HF packet consumers and recursive streams, budget-adaptive VarDCT caps, plus broader corruption/truncation fuzz coverage. | `ENT-D01` |
