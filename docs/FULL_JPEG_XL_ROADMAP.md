...
| `ENC-03` | P2 | **Missing** | Implement automatic Modular versus VarDCT selection and per-feature decisions. Explicit per-frame selection now exists under the common RGB8 original-sRGB sequence contract, with byte-identical forced alternatives and cross-codec composition evidence. This supplies the execution boundary, not an automatic policy. Completion evidence must show the selected path and compare it with forced alternatives. | complete baseline encoders |
