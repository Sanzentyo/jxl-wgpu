...
| `CONT-05` | P1 | **Partial** | Bounded `jbrd` parsing/emission now preserves marker groups, Huffman/quantization declarations, component selectors, scan/progression metadata, restart/reset/ZRL records, entropy padding and opaque APP/COM/intermarker/tail data. The immutable model validates grammar and exact Brotli consumption under marker/entry/decoded/owned limits; emission accounts for the parsed model, temporary compression and final payload together. A 36-input / 140-scan corpus includes three pinned official originals and sequential/progressive RGB, gray, subsampled, restart and preservation variants. Strict libjxl JPEG-only reconstruction agrees with the original JPEG bytes for unchanged input and three emitted compression settings (144 comparisons); malformed/missing records, exact budgets and truncation have negative evidence. This is metadata interoperability: actual GPU-decoded quantization/LF/AC binding, frame/metadata compatibility, GPU JPEG entropy and leased authoritative byte output remain required. Lossless JPEG ingestion is still `ENC-05`. [Contract](CONTAINER_METADATA.md#jpeg-reconstruction-metadata). | `VDCT-D03`, `VDCT-E02` |
