...
| `VDCT-D02` | P0 | **Partial** | The DCT8 pass-group executor now selects arbitrary valid HF block-context maps from stream-defined quant-field and signed X/Y/B LF thresholds entirely on GPU, with an actual-GPU scalar differential matrix. Completion still requires mixed strategy maps, all 27 regular and special strategies in mixed images, full quant fields, chroma-from-luma factors, and required Modular side images. No legal strategy may be lowered to DCT8. | `VDCT-D01`, `MOD-D05` |
