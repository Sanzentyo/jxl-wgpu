...
| `META-03` | P1 | **Partial** | Contiguous and incremental inventory preserve LF levels, resolve every `USE_LF_FRAME` read to the exact earlier producer in JPEG XL's four progressive-DC slots, reset dependency state across the preview/main boundary, and reject a missing producer with a typed error before submission. A libjxl `--progressive_dc=2` chain is checked under contiguous and one-byte event delivery. The stock scheduler executes one Modular LF producer into resident XYB and consumes it from the final VarDCT frame without readback; a `--progressive_dc=1` actual-GPU fixture matches Rust `jxl` within one RGB8 code and exposes only the final frame. Completion requires general single-entry intermediate LF packets for multi-level chains, preview decode, and explicitly non-final progressive outputs without corrupting main-frame state. | `FRONT-03`, `FRAME-03` |
...
logical blocking/poll/Future pending state are implemented. The completed one-level path converts
the Modular producer's final signed planes to three resident F32 XYB buffers, applies the exact LF
binary16 multipliers divided by 128, packs `[X,Y,B,0]` into the VarDCT LF atlas, retains both
physical lifetimes under the shared byte budget, and publishes only the final frame. The next P0
packet item is general HF-global plus AC decoding after GPU-decoded HF metadata in a single TOC
entry; without it, a `--progressive_dc=2` intermediate LF frame cannot yet feed the next level.
