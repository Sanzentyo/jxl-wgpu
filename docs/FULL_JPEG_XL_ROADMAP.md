...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `ENT-D01` | P0 | **Partial** | The bounded Modular, VarDCT packet, and DCT8 pass-group paths share a typed 12-byte Rust/WGSL `EntropyStreamParams` prefix for token bounds and the LZ ring mask while preserving consumer-specific records and bindings. Whole-range consumers also share exact ANS-state and zero-padding termination. The VarDCT packet layer now represents an absent global MA tree, host-packs each LF-local descriptor, executes LF entropy on GPU, maps only aggregate cursor/status records, parses each following HF-local descriptor, and resumes through a separate GPU entry point without host image entropy. An actual-GPU 2056×256 ordinary-`cjxl` test covers two LF groups. Completion requires connecting that two-stage core to the runtime-neutral frame-engine pending state and every remaining LF/HF consumer. | `FRONT-02` |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `VDCT-D01` | P0 | **Partial** | One LF-global/multiple-LF-group/HF-global topology now executes nonempty single-pass pass groups in scanline or arbitrary entropy-permuted physical order, including center-first. LF groups own independent packet/HF workspaces but scatter into shared full-image resident atlases and one output. A shared-global-tree 2056×256 fixture validates one submission and one aggregate status map. The packet layer separately proves the required two-stage local-tree entropy sequence against ordinary `cjxl`; production completion requires wiring its cursor map and second submission into the frame engine, then recursive LF frames and additional passes. | `FRONT-02`, `ENT-D01` |
| `VDCT-D02` | P0 | **Partial** | Sectioned packets now GPU-decode mixed strategy maps for all 27 regular and special strategies, capacity-strided quant fields, `hf_mul`, extra precision, per-frequency-cell HF chroma correlation, and arbitrary valid block-context maps selected from stream-defined quant-field and signed X/Y/B LF thresholds. Global LF/correlation/resource origins cover cross-group addressing. An actual-GPU scalar context matrix, a mixed libjxl image, and two standard multi-LF-group images cover the production path without lowering a legal strategy to DCT8. Completion requires local-tree Modular substreams, required Modular side images, and broader mixed-strategy/correlation conformance coverage. | `VDCT-D01`, `MOD-D05` |
...
| `VDCT-D05` | P0 | **Partial** | Every normative default strategy matrix, stream-selected global/LF/HF scales, extra precision, quant bias, DC prediction, and per-cell HF chroma correlation execute on GPU for the bounded global-tree profile. Multiple LF groups scatter into full-image LF/correlation atlases; adaptive LF smoothing runs once globally, while the skip flag uses a resident buffer copy. Matrix orientation is explicit for regular, wide, and special coefficient layouts. Completion requires custom matrices, non-default LF channel-correlation metadata, local MA trees, and Modular side images. | `VDCT-D02`, `VDCT-D03` |
...
multi-LF-group tiled-DCT8 LF-only encoder are implemented. The immediate P0 gap is frame-engine
integration of the implemented two-stage local per-substream MA-tree packet core. After that,
`VDCT-E02` must emit real
