...
| `ENC-02` | P1 | **Partial** | Explicit per-frame color factors 1/2/4/8 now share one checked geometry plan across Modular, VarDCT and mixed sequences. Reduced GPU sources, packed alpha, relative extra sampling, displayed crops/references and coded-grid memory/progression have [independent numerical and ownership evidence](CONFORMANCE_CORPUS.md#per-frame-color-and-relative-extra-sampling). This signals decoder reconstruction; it does not downsample full-resolution sources or establish a quality guarantee. Existing explicit codec/progressive/deterministic/memory controls remain bounded profiles. Complete lossless/lossy, distance/quality, effort, decoding-speed, automatic resampling, metadata-preservation and memory/latency policy controls with typed invalid combinations remain. | encoder mode rows |
