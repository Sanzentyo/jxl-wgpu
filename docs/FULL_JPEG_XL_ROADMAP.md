...
| `VDCT-D07` | P1 | **Partial** | The public VarDCT decoder executes JPEG reconstruction's per-component 2× horizontal/vertical upsampling with normative quarter/three-quarter weights and replicated odd borders inside the resident output pass. Completion requires pre-restoration scheduling, ordinary frame 2×/4×/8× color/extra-channel resampling, all JPEG sampling layouts, and group halos. | `MOD-D05`, `RENDER-01` |
