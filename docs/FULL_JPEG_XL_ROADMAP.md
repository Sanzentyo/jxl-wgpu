...
GPU consumers still start from contiguous codestream ownership. The immediate P0 gap remains the
common frontend and entropy work in `FRONT-01` and `ENT-D01/02`: connect section events to the
existing upload windows and add recursive-stream resume. In parallel, `VDCT-E02` must emit real
