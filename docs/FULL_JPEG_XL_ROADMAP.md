...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `ENT-D01` | P0 | **Partial** | The bounded Modular, VarDCT packet, and DCT8 pass-group paths share a typed 12-byte Rust/WGSL `EntropyStreamParams` prefix for token bounds and the LZ ring mask while preserving consumer-specific records and bindings. Whole-range consumers also share exact ANS-state and zero-padding termination. The VarDCT frame engine represents an absent global MA tree, host-packs each LF-local descriptor, executes LF entropy on GPU, maps only aggregate cursor/status records, parses each following HF-local descriptor, and resumes through a separate GPU entry point without host image entropy. Oversized staged LF-local streams now execute as ordered bounded windows and expose only their final aggregate end cursors. Its runtime-neutral pending state owns every LF submission and both maps. An actual-GPU 2056×256 ordinary-`cjxl` test covers two LF groups through blocking and async completion. Completion requires the HF and combined/global packet consumers, recursive LF frames, and incremental input delivery. | `FRONT-02` |
| `ENT-D02` | P0 | **Partial** | Every accepted stock Modular MA profile, staged local-tree VarDCT LF packets, and the VarDCT AC pass-group consumer split oversized entropy into ordered GPU work over one reusable upload. Adjacent segments carry 16-byte backward/forward overlap and yield only between complete output tokens. Modular uses 32/48/112-byte aligned resume records. VarDCT LF packets define 64-byte generic and 128-byte SelfCorrecting `Pod` states inside each reconstruction allocation; current local-tree groups conservatively reserve 128 bytes because their HF tree is discovered only after LF completion. VarDCT AC uses a 464-byte `Pod` record holding common ANS/LZ state, nested block/channel/coefficient progress, sticky sink failure, and the three-channel 96-word nonzero-neighbour grid. An explicit bounded-mode bit keeps middle packet windows distinct even when neither FIRST nor FINAL is set. Channel boundaries reset predictor-local state; only a final segment performs exact ANS/padding termination. The shared stream cap is caller-configurable and bounded by device limits; Modular additionally adapts it against the per-frame budget. Host scheduling tests cover unaligned starts, overlap mapping, lane isolation, budget adaptation, undersized caps, and segmentation of every oversized Modular profile. Actual-adapter fixtures force 193×97 Prefix+RLE/LZ77 Gradient, libjxl 193×197 ANS Weighted, libjxl 2056×256 local-tree LF packets, and libjxl 438×589 nonzero/custom-order VarDCT AC through 256-byte caps. Blocking and runtime-neutral async outputs match their source/Rust-`jxl` oracles, abandonment releases reservations, and existing Modular late-window damage returns typed group-specific GPU entropy failure from the final aggregate map. Memory stats expose selected state/window/batch sizes without double-counting packet state. Completion still requires bounded resume for VarDCT HF and combined/global packet consumers and recursive streams, budget-adaptive VarDCT caps, plus packet-level and broader corruption/truncation fuzz coverage. | `ENT-D01` |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `VDCT-D01` | P0 | **Partial** | One LF-global/multiple-LF-group/HF-global topology executes nonempty single-pass pass groups in scanline or arbitrary entropy-permuted physical order, including center-first. LF groups own independent packet/HF workspaces but scatter into shared full-image resident atlases and one output. A shared-global-tree 2056×256 fixture validates one submission and one aggregate status map. Ordinary `cjxl` per-substream local trees execute through the production frame engine as one or more bounded LF submissions, one aggregate LF map, one whole-range HF/downstream submission, and one final aggregate map. A generated 2056×256 effort-7 stream with a forced 256-byte cap exercises exact submission/memory accounting, blocking, runtime-neutral async, typed early-handoff refusal, and cancellation memory release. Completion requires bounded HF/combined packets, recursive LF frames, additional passes, and broader legal packet topologies. | `FRONT-02`, `ENT-D01` |
...
The coding-mode selector, shared typed entropy-stream ABI, bounded Modular/VarDCT-AC/staged-LF
...
common frontend and entropy work in `FRONT-01` and `ENT-D01/02`, including HF/combined packet and
recursive-stream resume plus incremental input rather than whole-input retention. In parallel, `VDCT-E02` must emit real
