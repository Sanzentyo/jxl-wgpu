...
| Lossless Modular encode | **Partial** | Gray/GrayAlpha/RGB/RGBA with 1–31-bit integers or IEEE binary16/binary32, packed/planar/split buffers, component swizzles and explicit word bit/byte order, selectable 128/256/512/1024 groups, one pass, fixed Gradient and prefix+RLE/LZ77. Integer RGB uses YCoCg; floating channels retain raw words without a transform. Enumerated or original embedded RGB/Gray ICC color, all intents, explicit image white and source alpha association are retained; ICC headers have checked byte limits and shared-budget lifetime. Exact original-word and native/GPU output evidence covers high depths, IEEE special values and invisible colors; finite animations cover all five blend modes, independent color/alpha references and signed crops. |
...
256-pixel groups retain the checked-in Gray8 bytes. `MOD-E02` becomes **Partial**; transform selection/topology
...
| `MOD-E02` | P1 | **Partial** | All four standard Modular group sizes (128/256/512/1024) now share checked GPU geometry, LF grids, frame headers and resident/streamed admission. Independent exact integer/IEEE/native comparisons, full zero/dense tiles, both tree modes, cropped/reference animations and cancellation cover selection; default 256 streams retain their bytes. Complete local/global RCT selection, Palette/delta palette, Squeeze and corresponding global/LF/HF topology remain. Round trips must cover every transform and composition order. | `ENT-E01`, `MOD-D03` |
