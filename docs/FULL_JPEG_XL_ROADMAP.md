...
Status date: 2026-09-20. This document is the canonical capability and implementation backlog for
...
| JPEG bitstream reconstruction | **Partial** | Bounded immutable `jbrd` metadata parsing and canonical emission have original-JPEG native interoperability evidence. GPU frame/coefficient binding, JPEG entropy execution and authoritative JPEG byte output remain absent. |
...
| `CONT-05` | P1 | **Partial** | Bounded immutable `jbrd` parsing/emission preserves marker/table/component/scan/progression declarations, restart/reset/ZRL and padding records, and opaque metadata under exact Brotli and logical-byte limits. The 36-input / 140-scan corpus has 144 independent libjxl original-JPEG byte comparisons across unchanged and re-emitted metadata. A separate GPU coefficient API now captures exact raw quantizers and restores actual decoded LF/AC, integer DC/CfL, component order and MCU padding into one validated, budget-owned lease. Original libjpeg-turbo coefficients agree exactly for all 36 pairs, including three official originals; bounded-window asynchronous output, integer limits, malformed state, one-byte-short admission/retry, cancellation and retained-output ownership have evidence. The qualified still DCT8 profile remains bounded: complete scan/external-metadata compatibility, GPU JPEG entropy, authoritative original-byte output and broader legal variants remain required. Lossless JPEG ingestion is still `ENC-05`. [Metadata contract](CONTAINER_METADATA.md#jpeg-reconstruction-metadata); [coefficient contract](../crates/jxl_wgpu_decode/README.md#gpu-jpeg-reconstruction-inputs). | `VDCT-D03`, `VDCT-E02` |
