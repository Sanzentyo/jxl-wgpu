...
| `ENC-04` | P1 | **Partial** | VarDCT accepts up to 256 resolved lossless scalar planes, including packed alpha, with independently checked integer/floating precision, intrinsic dimension shifts 0–3, Alpha/Depth/SpotColor/SelectionMask/Black/CFA/Thermal/Optional metadata and bounded UTF-8 names. One plan owns global-prefix/LF/pass routing, source windows, artifact identities and memory bounds. Still/sequence APIs preserve source samples, support each extra blend/reference and explicit alpha selectors, and reject mismatched input before admission. [Independent-word, native/GPU, metadata and ownership evidence](CONFORMANCE_CORPUS.md#independent-vardct-extra-input) covers this subset. Independent Modular/mixed extras, per-channel lossy distance, additional frame upsampling policy and broader invisible-color/semantic conformance remain. | `ENC-01`, `COLOR-03` |
