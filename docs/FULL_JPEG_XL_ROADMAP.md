...
| VarDCT decode | **Partial** | A separate authoritative 8-bit XYB engine covers mixed maps containing all 27 regular and special transform strategies, single-pass nonzero AC, all 13 natural/custom coefficient-order families, stream-defined block contexts and HF chroma correlation, every normative default dequant matrix and 3-bit X/B scale, scanline or entropy-permuted center-first pass groups, multiple LF groups with shared or per-substream local MA trees, resident Gaborish plus one-to-three-iteration EPF, and a checked 2056×256 LF-boundary extent. |
...
| `VDCT-D05` | P0 | **Partial** | Every normative default strategy matrix, all 3-bit stream-selected X/B scales, stream-selected global/LF/HF scales, extra precision, quant bias, DC prediction, and per-cell HF chroma correlation execute on GPU for the bounded global- and local-tree profiles. Multiple LF groups scatter into full-image LF/correlation atlases; adaptive LF smoothing runs once globally, while the skip flag uses a resident buffer copy. Matrix orientation is explicit for regular, wide, and special coefficient layouts. Completion requires custom matrices, non-default LF channel-correlation metadata, and Modular side images. | `VDCT-D02`, `VDCT-D03` |
