...
| `QA-02` | P0 | **Partial** | The official animation_spline (60 frames), cmyk_layers and twelve additional stills now use untouched content-addressed references with every published per-channel RMSE/peak bound. The stills cover lossless F32, both alpha associations, extended signed color, spot planes, layered orientation, blend modes, delta palettes, LZ77, lossless patches and Gray output; whole and bounded fragmented output agree exactly. Gray JPEG pixel evidence does not establish JPEG byte reconstruction, and retained generated ICC references do not establish exact generated-profile export. Input, descriptor, profile and reference hashes plus metadata/shape are checked. Zero-error F32 additionally requires exact IEEE words. Original numeric ICC samples do not imply an accepted color profile. Completion requires the remaining ISO/IEC 18181-3 suite or an auditable equivalent; these cases do not establish full decoder conformance. | full decode path |
