...
| `FRONT-03` | P1 | **Partial** | Transport accepts arbitrary owned chunks without collecting a whole stream: apart from the inline signature, ordered raw/`jxlc`/v0-`jxlp` payloads share caller `Arc` storage and gap-blocked v1 fragments use a separately limited retained buffer. `CodestreamStreamScanner` then reconstructs only bounded image/frame-header/TOC probes, emits `Arc` frame inventories before section data, and routes physical section ranges in logical codestream order. The Modular metadata reader and bounded uploader no longer require one physical host slice: a checked shared-span source reads bits and copies upload ranges across every byte/entropy-word split without whole-codestream reassembly. Byte-drip animation, entropy-permuted TOCs, v1 reorder, truncation, bounded prefix retention, shared section backing, span gaps/overlaps, and every Modular fixture split are covered. Completion still requires connecting section events to that Modular boundary, converting VarDCT to the same source contract, bounding source-span lifetime with GPU backpressure/cancellation, and removing the contiguous `GpuDecoder::open` prerequisite. | `CONT-01` |
...
GPU consumers still enter through contiguous public ownership, although Modular metadata and
uploads now share a chunk-capable source contract internally. The immediate P0 gap remains the
common frontend and entropy work in `FRONT-01` and `ENT-D01/02`: connect section events to that
Modular boundary, convert VarDCT, bound retained span lifetime, and add recursive-stream resume. In
