...
| `CONT-03` | P1 | **Partial** | Bounded plain `jxli` parsing/emission binds exact logical offsets, displayed-frame spans and rational time to complete headers. Generated conservative anchors drive `open_seek`/`open_seek_shared`; transitive LF, color/extra/alpha and patch-slot dependencies restore required reference versions before target output. Whole and 256-byte-window GPU output matches sequential pixels and original timing, with independent native pixels and index-producer evidence. Malformed indexes, required-entropy failures, retry, cancellation and retained progressive output are covered. Complete input is required; incremental/range-based seeking, compressed-index policy and broader container/feature conformance remain. [Contract and evidence](FRAME_SEEKING.md). | `CONT-01`, `FRAME-04` |
...
| `FRAME-04` | P1 | **Partial** | One-presentation GPU seek/restart now preserves original physical IDs, noise seeds, reference-saving color semantics, duration/timecode and finality while suppressing bounded preroll. Required older slot versions can rewind across a later independent anchor; validated target LF/pass images retain ordinary immutable leases. Header-only dependency probes and real-GPU mixed/composed/recursive-DC, patch/extra, rejection and lifetime tests cover the current boundary. Non-coalesced frame/layer output, broader skip controls and incremental/range-based seeking remain. [Seek contract](FRAME_SEEKING.md). | `FRAME-03`, `CONT-03` |
