...
| Lossless Modular encode | **Partial** | Gray/RGB/RGBA with 1–31-bit integers or IEEE binary16/binary32, 256×256 groups, one pass, fixed Gradient and prefix+RLE/LZ77. Integer RGB uses YCoCg; floating channels retain raw words without a transform. Standard animation is implemented. Exact original-word and native/GPU output evidence covers high depths, IEEE special values, full Replace and finite crop/Add/Multiply animations. |
...
color/ICC declarations, other layouts, learned transforms/predictors and progressive encoding
remain open.
...
| `IO-02` | P1 | **Partial** | Packed native U8/U16/U32 Gray/RGB/RGBA buffers encode every 1–31-bit integer depth, including ignored high padding bits; native binary16/binary32 preserve every IEEE source word. Both accept unaligned padded rows. Other floating precisions and broader portable RGB/BGR/YUV buffers and textures remain; normalize planar/semi-planar formats and bit alignment in WGSL with no host pixel conversion. | `COLOR-01`, encoder modes |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `ENC-01` | P1 | **Partial** | Lossless Modular accepts all 1–31-bit packed integer and IEEE binary16/binary32 Gray/RGB/RGBA sources with exact original-word conformance and unassociated alpha. Floating samples retain special values and payloads through shared/local trees, Replace animations and bounded streaming. Other floating precisions, alpha association, arbitrary extra-channel planes, color/ICC metadata, planar buffers and textures remain; all input normalization must stay on GPU. | `IO-02`, `COLOR-03` |
