...
Status date: 2026-09-26. This document is the canonical capability and implementation backlog for
...
| VarDCT decode | **Partial** | The authoritative color-output engine accepts 1–31-bit integer or legal floating XYB/original-sRGB metadata, or 8-bit integer YCbCr input, normalizes all eight orientations for RGB/gray presentation and covers mixed XYB maps containing all 27 regular and special transform strategies, nonzero AC across spectral/refinement passes, opt-in immutable DC/AC images for stills, animations and composed presentations including extra channels and deferred descriptors, plus LF dependency images with independent alpha/extra output, all 13 natural/custom coefficient-order families, stream-defined block contexts, non-default LF dequantization and LF/HF chroma correlation, every normative default and parametric custom strategy matrix, all 3-bit X/B scales, scanline or entropy-permuted center-first pass groups, multiple LF groups with shared or per-substream local MA trees, recursive progressive-DC dependencies, resident Gaborish plus one-to-three-iteration EPF, standard/custom 2×/4×/8× frame resampling, and a checked 2056×256 LF-boundary extent. Sectioned raw mode-7 matrix side images reuse bounded GPU uploads with deferred inverse/overlay completion. Both global- and local-MA raw images, including independent local LF/HF packet trees without a global tree, have actual-adapter evidence through 40-byte windows and fragmented input. Checked non-XYB 4:4:4/4:2:2/4:4:0/4:2:0 JPEG-transcode streams use component-sized LF/AC grids, resident quarter/three-quarter JPEG upsampling, and encoded YCbCr conversion through public `GpuDecoder`. Shifted components also have budgeted expansion before restoration, frame resampling, nonzero noise or retained component references; twenty JPEG restoration/noise streams now validate those expanded planes with independent scalar and CPU references. Another 128 streams cover all 64 component sampling selector triples at aligned and odd extents, including nonzero/zero noise and active adaptive LF for equal factors. Eight additional nonzero-LF-correlation streams fix the equal-nonzero-selector case: LF correlation now uses normalized component shifts. Unequal-factor adaptive LF is rejected as malformed by the shared header parser before TOC/section/GPU work; all four equal triples remain valid. Raw matrices for all 17 families now have GPU encoder/native pixel evidence; broader transformed raw-matrix, precision and asymmetric restoration/resampling conformance remain. |
| Lossless Modular encode | **Partial** | [Independent scalar inputs and mixed integration](CONFORMANCE_CORPUS.md#independent-modular-and-mixed-input) now share checked Global/LF/Pass topology, sampling and resource ownership.  Gray/GrayAlpha/RGB/RGBA with 1–31-bit integers or all 154 legal floating precisions (2–8 exponent and 2–23 fraction bits), packed/planar/split buffers, component swizzles and explicit word bit/byte order, selectable 128/256/512/1024 groups, one pass, all 14 explicit predictors (including custom Weighted/SelfCorrecting coefficients) and default Prefix or clustered GPU ANS with adaptive residual/distance/length hybrid coding and 64/128/256-symbol alphabets with zero runs or GPU greedy LZ77 search for arbitrary residual sequences. RGB(A) selects any of 42 RCT types globally or per group, or no transform; defaults remain YCoCg for integers and independent raw words for floats. Exact local GPU color, delta, mixed or implicit Palette composes after RCT on any selected contiguous component range, with independent predictor and entry limits. Ordered local RCT/Squeeze programs additionally address any three equal-geometry current image channels within the 273-entry header limit. Explicit local horizontal/vertical Squeeze, including both axis orders, selects a post-Palette image-channel range and tail/in-place residual placement, with signed-wide arithmetic and typed rejection of unrepresentable residuals. CMYK ICC input adds exact primary Black and explicit sample conventions through the [shared input plan](CONFORMANCE_CORPUS.md#cmyk-encoder-input). Enumerated or original embedded RGB/Gray/CMYK ICC color, all intents, explicit image white and source alpha association are retained; ICC headers have checked byte limits and shared-budget lifetime. Exact original-word and native/GPU output evidence covers high depths, IEEE special values and invisible colors; finite animations cover all five blend modes, independent color/alpha references and signed crops. |
| VarDCT encode | **Partial** | All 27 strategies perform real forward transforms, normative LF extraction, default or caller-selected parametric/raw matrix AC quantization, natural or caller-selected per-channel orders and prefix packing on GPU. Caller-selected mixed maps validate exact coverage and AC-group boundaries, batch transforms by strategy, replicate non-DCT8 edges and assemble multiple LF/AC groups. Tiled DCT8 retains its workgroup path and checked 16K axis bound. Independent f64/native coefficient and Rust/libjxl/GPU image tests cover mixed maps and group boundaries under five byte-identical variants. Spectral and quantized AC encoding supports 1–11 passes with exact final-coefficient convergence and native intermediate-image evidence. GPU local-contrast statistics now select deterministic AC-group delivery order without pixel readback. Integer (1–31-bit) or floating (all 154 legal precisions) Gray/GrayAlpha/RGB/RGBA with enumerated SDR/HDR or embedded ICC input in XYB or original-component stills and animations share checked frame control with Modular and support timed signed crops, all five blend modes with alpha, independent color/alpha reference fields, hidden regular frames and four post-color-transform references. ICC XYB uses a relative GPU connection to linear BT.709/Gray; forbidden post-transform references are rejected before admission. Shared bounded ICC headers, independent coefficient/pixel references and exact memory ownership are covered in the [ICC input corpus](CONFORMANCE_CORPUS.md#icc-vardct-and-mixed-input). [Lossless alpha evidence](CONFORMANCE_CORPUS.md#alpha-vardct-and-mixed-input) covers same-precision raw words, progression and composition; [independent extra inputs](CONFORMANCE_CORPUS.md#independent-vardct-extra-input) add separate precision/geometry, metadata and selected blend weights through one image-wide routing plan. [CMYK input](CONFORMANCE_CORPUS.md#cmyk-encoder-input) adds profile-bound four-component input, an exact primary Black plane, Original/XYB coefficient evidence and mixed/preview integration. Content-adaptive transform/quantizer selection, general distance/quality guarantees, rate control, DC progressive encoding and broader saliency evidence remain absent. |
...
| `CONT-03` | P1 | **Partial** | Bounded plain `jxli` parsing/emission binds exact logical offsets, displayed-frame spans and rational time to complete headers. The shared immutable `FrameSequencePlan` drives decoder execution, index binding and opt-in encoder `jxli` emission from actual assembled headers. Generated conservative anchors drive `open_seek`/`open_seek_shared`; transitive LF, color/extra/alpha and patch-slot dependencies restore required reference versions before target output. `FrameIndexCollector` and `stream_seek` collect bounded metadata alongside shared input spans, require authoritative End, and transfer ownership without joining the host codestream. Whole and 256-byte-window GPU output matches sequential pixels and original timing, with independent native pixels and index-producer evidence. Malformed indexes, required-entropy failures, atomic input admission retry, cancellation, independent previews and retained progressive output are covered. Complete input is required; byte-range seeking, compressed-index policy and broader container/feature conformance remain. [Contract and evidence](FRAME_SEEKING.md). Encoder `jxli` now selects the main image through the shared bitstream image-selection boundary, retaining preview-relative physical offsets; mixed preview/main sequences have native output and GPU seek evidence. | `CONT-01`, `FRAME-04` |
...
| `ENT-E01` | P1 | **Partial** | Modular has explicit greedy GPU LZ77 search over arbitrary residual sequences, overlapping and regular-distance matches, GPU literal/length/distance histograms, checked artifacts and canonical Prefix metadata. A bounded 32-candidate hash chain preserves Weighted state. Default Prefix/zero-run bytes remain stable. Explicit ANS serializes complete groups on GPU with frame-wide 4096-slot distributions and shared state across transformed channels. One immutable codebook owns all wire/GPU choices: one to five shared distributions across four channel contexts and distance, residual/distance hybrid settings, global LZ77 length split, minimum symbol and alias alphabet. Joint deterministic rate selection covers all 52 context partitions, 40 full-u32-safe hybrid configurations and 94 legal threshold/length domains with 64/128/256-symbol alphabets, including exact extra bits and repeated header costs. Normalized candidate metadata remains separate from final GPU alias tables and admitted worst-case storage. Independent exhaustive alias/cursor/state, partition/rate, full length-domain and threshold-boundary checks complement 394 actual-GPU length/raw/LZ combinations. Public native/original-word and whole/bounded GPU evidence covers every depth/predictor/RCT, Palette/programs, animation, selected alphabets/length headers, malformed state, exact budgets, cancellation and late overflow; see [ANS coverage](CONFORMANCE_CORPUS.md#lossless-modular-gpu-ans-encoding). Learned/wider contexts, clustering outside Modular ANS, input-domain/effort policies and cross-channel search remain. `djxl`/`jxl` must accept every generated family. | — |
...
Independent scalar inputs now span complete Modular and mixed sequences as one design unit.
`ImageSamplePlan`, the common checked input windows and scalar routing, and the Modular transform
plan keep declarations, physical grids, local topology and resource ownership distinct. Up to 256
resolved extras include packed alpha; every integer/floating precision remains exact before
resampling. Prefix/ANS, color/extra factors, local transforms, crops/references, all blend modes,
fragmented GPU output and exact admission/cancellation have
[independent evidence](CONFORMANCE_CORPUS.md#independent-modular-and-mixed-input).
Native libjxl retains its effective-factor-eight limit; extended factors use unchanged independent
Rust and F64/GPU bounds. `ENC-01/04`, `IO-02`, `MOD-E02` and the full goal remain **Partial** for
CMYK/YUV/texture input, per-extra lossy policy, general/global transform placement and broader
encoder quality/conformance work.
...
The [custom floating checkpoint](CONFORMANCE_CORPUS.md#custom-floating-point-modular-encoding)
extends input precision to every legal 2–8 exponent / 2–23 fraction-bit combination, closing the
earlier non-IEEE precision gap. Shared `FloatPrecision` owns the checked declaration; packing,
source plans, still/alpha headers and animation identity use that declaration without a new GPU
conversion path. The 308 Prefix/ANS still cases require independent exact original words and
native/GPU F32 output. Transform combinations, four Replace animations, IEEE byte compatibility,
invalid admission and resident/streamed ownership provide additional evidence. `ENC-01` and
`IO-02` remain **Partial** for CMYK, YUV/texture input and remaining encoder work.
...
The [alpha-input checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-alpha-input) adds GrayAlpha
and explicit associated/unassociated source declarations without changing source samples or the
GPU ABI. Both tree modes and all 1–31-bit integer/binary16/binary32 depths retain original words,
including invisible colors and IEEE special values. Sixteen six-frame sequences cover all blend
modes, separate color/alpha references, signed crops, timing and retained whole/bounded output
against libjxl and Rust jxl. Final Preserve/Unassociated/Associated presentation and exact
resident/streamed admission/cancellation are covered separately. `ENC-01`, `IO-02`, `FRAME-05`
and `QA-03/06` remain **Partial**; independently declared extra precisions/types, CMYK,
YUV/textures and the remaining encoder syntax/quality work stay open.

The [embedded-ICC checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-embedded-icc) preserves
original RGB/Gray profile bytes through integer/IEEE stills and Replace animations, with ordinary
or explicit device-channel layouts. Native original-profile export and original-device samples,
independent retained working words, requested scalar color conversion and whole/bounded GPU output
cover the contract. Profile/intent identity and size limits validate before variable-sized header
allocation or GPU admission. Header storage joins the shared byte budget with separate still,
animation, completion and cancellation lifetimes; the GPU ABI and submission counts are unchanged.
`ENC-01`, `IO-02`, `FRAME-05`, `API-05` and `QA-03/06` remain **Partial**. CMYK inputs,
independent precision, broader ICC composition and remaining encoder syntax/quality stay open.

The [group-size checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-group-sizes) makes all four
128/256/512/1024 edges selectable through `LosslessModularConfig`. Frame/LF grids, source/artifact
limits, streamed batches and the standard frame header share one size declaration, including each
animation crop. Native/Rust original-word and whole/bounded GPU evidence covers 128 integer/IEEE
still cases, full zero/dense tiles through 1024², Replace and cropped/composed animations, exact
admission and cancellation. Long zero-run tokens are checked through the largest group. Default
256-pixel groups retain the checked-in Gray8 bytes. `MOD-E02` remains **Partial**; transform stacks/topology
and the remaining encoder items retain their separate acceptance gates.

The [RCT checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-rct-selection) adds explicit no-transform
and all 42 standard RCT types, declared globally or in each pass group independently of MA-tree
placement. Integer and raw IEEE source words use wrapping GPU arithmetic, including permutations
and high-depth average overflow; alpha stays independent. The 1,008-case exact/native/GPU matrix,
all-depth no-transform/identity checks, Replace and cropped/reference animations, local-header
inspection, typed invalid selections, exact admission and cancellation cover the selection policy.
The configuration is immutable across groups and animation frames; automatic content-dependent
selection and arbitrary RCT/Palette/Squeeze stacks remain. `MOD-E02` and the full goal stay **Partial**.

The [Squeeze checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-squeeze-encoding) adds explicit
local horizontal/vertical transforms and both separable axis orders after RCT. GPU signed-wide
rounding, single-pixel-axis elision and variable channel admission preserve exact source words
within the representable signed-32 residual domain. Typed overflow returns no stream. Native/Rust
oracles, whole/fragmented GPU output, all predictors/group sizes, animation, exact budgets,
cancellation and late streamed failure cover this scope. General stacks and global/LF topology
remain open; no complete transform or full JPEG XL claim is made.

The [Palette checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-palette-encoding) adds bounded
exact local dictionaries built on GPU after RCT, optionally followed by Squeeze of the index
image. Every color-count wire bucket through 70,911, full integer/IEEE words, predictor/RCT
composition, both tree modes, animations and whole/bounded GPU output have independent evidence.
Capacity overflow publishes no stream, including late streamed failures; dictionary/hash scratch
obeys exact admission, cancellation and pool reuse. Arbitrary ordering and global/LF transform topology remain open. `MOD-E02` stays **Partial**.

The [Delta Palette checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-delta-palette-encoding)
adds dictionaries of complete-component predictor residuals, with all 14 predictors independent
of entropy prediction and all explicit counts through 66,816. The pinned scalar libjxl exports
original integer planes for exact comparison where jxl-oxide's delta-table handling is unsuitable.
Native F32 and whole/bounded GPU output retain their existing bounds; animation, overflow and
resident/streamed lifetime cover the added residual and Weighted storage. A smooth high-color
source fits four delta entries and beats its ordinary Gradient stream.
The [mixed Palette checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-mixed-palette-encoding)
adds separate caller-bounded absolute and residual partitions, with absolute-match priority and
compact delta-then-color wire tables. Exact native words, unchanged F32/GPU bounds, all predictors,
integer/IEEE/RCT/Squeeze composition, both simultaneous count maxima, independent header checks,
animation and resident/streamed lifetime cover the policy. General stacks and adaptive selection remain; `MOD-E02` and the full goal stay **Partial**.

The [implicit Palette checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-implicit-palette-encoding)
adds exact whole-tuple cube selection and canonical signed delta lookup before explicit residuals.
One declared zero delta avoids native single-channel index clamping; cube selection uses depths
1–24 where native and normative scaling agree, while wider integers/IEEE words retain signed
implicit or explicit residuals. Native entry exports and pre-inverse index counts independently
verify actual use, with original-word, F32, animation, overflow and resource-lifetime checks.
Shared WGSL entry constants preserve existing decoder arithmetic. Table-free
implicit policies, general stacks/topology, adaptive choices and other encoder gates remain open.

The [component-selection checkpoint](CONFORMANCE_CORPUS.md#lossless-modular-palette-component-selection)
extends all four Palette policies to any nonempty contiguous post-RCT component range. Table and
residual storage scale with the selected count; the index and unchanged remaining components feed
both Squeeze axis orders. Native words and index audits, independent headers, all source depths,
animations and admission/cancellation cover the new topology. Streamed jobs reject an insufficient
...
Ordered [local RCT/Squeeze programs](CONFORMANCE_CORPUS.md#ordered-local-rctsqueeze-programs)
now extend this plan to equal-geometry triples in the evolving image-channel topology. They share
the reusable GPU arena, support all 273 local header entries and preserve prelude placement,
independent word/header checks and admission/lifetime contracts. Palette interleaving and
global/LF/HF topology remain open; `MOD-E02` stays **Partial**.
...
| `MOD-E02` | P1 | **Partial** | All four standard Modular group sizes (128/256/512/1024) share checked GPU geometry, LF grids, frame headers and resident/streamed admission. Caller-selected no-transform or any of 42 RCT types now operates on integer/raw IEEE RGB(A), with global or per-pass-group declaration independent of MA-tree placement. Exact/native/GPU comparisons, both tree modes, cropped/reference animations and cancellation cover selection; defaults retain their bytes. Explicit local horizontal/vertical Squeeze and both separable axis orders compose after RCT/Palette on all or selected contiguous image channels, with tail/in-place residual placement, per-group single-pixel-axis elision, signed-wide rounding and typed overflow before publication. A shared checked transform plan tracks selected descendants and unselected channel geometry; independent wire/native/GPU checks cover every range and placement, untouched IEEE extremes, invalid admission and resident/streamed ownership. Exact/native/GPU tests cover all source precisions within the representable domain, all predictors/group sizes, both entropy policies, animation and resident/streamed lifetime. Exact local GPU Palette now builds selected-component dictionaries after RCT, with checked 1–70,911 color limits, optional index-channel Squeeze, independent oracle/GPU comparisons and typed resident/streamed overflow. Explicit delta dictionaries now support all 14 independent predictors, raw integer/IEEE words, exact native working-plane comparison and budgeted residual/Weighted scratch. Mixed dictionaries now retain separately bounded absolute and residual entries with validated counts, exact native/GPU evidence through both simultaneous maxima, and resident/streamed lifetime. Implicit policies now select exact cube or signed delta entries before explicit residuals, with a declared zero delta and depth-aware native interoperability, independent index auditing and whole-word/lifetime evidence. All four Palette policies accept every contiguous post-RCT component range, preserving unselected image channels through Squeeze and independently checked words, headers, implicit indices, animations and resource ownership. Explicit ordered Squeeze sequences now add repeated axes and arbitrary current-image-channel ranges/placements after RCT/Palette. The shared plan owns checked shifts, empty residual slots, wire steps and disjoint reusable GPU arenas; upload, dispatch and admission consume that plan. Nine GPU tests cover all parameter-count/begin buckets through 296 steps and begin 1,096, exact native/GPU words, legacy byte identity, intermediate rejection/overflow, animation and resident/streamed ownership. Wide stacks also exercise empty-residual inverse aliasing and consecutive dispatches in one compute pass. Ordered local RCT/Squeeze programs now select arbitrary equal-geometry image triples, including alpha, Palette indices and earlier residuals. Their common plan owns operation order, dimensions/shifts, the 273-entry complete-header bound and 64-byte GPU jobs with disjoint lifetime-colored storage. Eight GPU tests cover every RCT type, source precision, Palette policy, header-count and RCT-begin bucket, exact native/GPU output, invalid admission, later-stage overflow, animation and resident/streamed lifetime. Arbitrary Palette placement and corresponding global/LF/HF topology remain, along with adaptive per-group choices. Round trips must cover every transform and composition order. | `ENT-E01`, `MOD-D03` |
| `MOD-E03` | P1 | **Partial** | All 14 caller-selected predictors now execute on GPU with signed-wide intermediates, arbitrary legal Weighted/SelfCorrecting header parameters, shared/local MA declarations and bounded per-group row state. Integer/IEEE exact-word and native/GPU matrices, all group sizes, RCT combinations, cropped/reference animations, streaming admission and cancellation provide evidence. A shifted-row source verifies a smaller NE stream than the unchanged Gradient baseline. Learned MA trees, previous-channel properties, weighted-parameter search, adaptive predictor choices and bounded effort tiers remain. | `ENT-E01`, `MOD-D02` |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `VDCT-E01` | P0 | **Partial** | All 27 strategies now perform real forward transforms and normative LF extraction on GPU. `VarDctStrategy` is the shared `TransformKind`, with no duplicate strategy alphabet or fixed diagnostic kernel. Regular transforms use separable storage passes; special transforms use bounded strategy-only bases. Correct LF comes from the raw LLF rectangle and resampling factors. All 667 pinned native coefficient/LF cases, including complete 8x8 impulse bases, match under five byte-identical linear variants. Fifty-four textured single-transform cases cover default/custom correlation, independent f64/native AC checks and native/Rust/GPU decode agreement within one RGB8 code. Tiled DCT8 retains odd-edge replication, 2048-pixel LF and 256-pixel AC groups through the checked 16K axis bound, with 2057x2057 nonzero AC and exact-black 16K panoramas. Caller-selected mixed maps now validate coverage, overlap, bounds and AC-group ownership before dispatch; per-strategy batches share image-wide arenas and variable-capacity AC slots. Native/f64 AC checks and three-decoder agreement cover all 27 strategies in a 512x512 map, a 2057x17 LF boundary and 13x21 non-DCT8 replicated edges, with five byte-identical variants. Batched native transform/LF tests cover disjoint/reordered ranges and poisoned guards. A shared color plan now supports XYB or original-sRGB coding without new allocations; 27 original-RGB strategies add independent AC and native/Rust/GPU F32 evidence, with tiled/mapped progressive variants and unchanged ownership bounds. See the [original-RGB corpus](CONFORMANCE_CORPUS.md#original-rgb-vardct-encoding). A shared sample plan now accepts canonical 1–31-bit integer RGB words, with matching GPU normalization and image metadata, explicit i32 LF buffers, all-depth coefficient/pixel evidence and wide-word checks for every transform; see the [integer-input corpus](CONFORMANCE_CORPUS.md#integer-rgb-vardct-input). The same plan now admits all 154 legal floating precisions with GPU field conversion and complete nonfinite validation before publication, per-workgroup status inside the existing artifact/readback, 924 topology/precision images, 54 all-strategy images and independent lifetime evidence; see the [floating-input corpus](CONFORMANCE_CORPUS.md#floating-rgb-vardct-input). [Gray input](CONFORMANCE_CORPUS.md#gray-vardct-and-mixed-input) now shares this checked plan and header syntax, aliases one source component on GPU without extra buffers, and covers all precisions, all strategies and source/sequence ownership at unchanged bounds. [Enumerated source color](CONFORMANCE_CORPUS.md#enumerated-vardct-and-mixed-input) now binds standard/custom primaries and white, seven wire transfers, four intents and exact image white to the same plan, with shared GPU transfer/Bradford conversion and independently checked metadata, coefficients, pixels and ownership. Completion still requires broader scalar/ISO conformance and source-domain coverage; content-adaptive strategy search remains VDCT-E04. Embedded RGB/Gray ICC input now uses shared bounded headers, relative GPU working conversion or original device samples, and the checked F.2 reference boundary; see the [ICC corpus](CONFORMANCE_CORPUS.md#icc-vardct-and-mixed-input). | `VDCT-D04` |
| `VDCT-E02` | P0 | **Partial** | All 27 strategies, including caller-selected image-wide mixed maps, and tiled DCT8 quantize real AC with shared default or caller-selected parametric/raw matrices (modes 0–7, all 17 families), natural or caller-selected X/Y/B orders for all 13 size classes and one prefix cluster for all 495 contexts, with LZ77 disabled. Native metadata verifies every order exactly and all AC matrix entries within 3e-6 relative error; the largest rectangular Y/B defaults are corrected in both encoder and decoder. Custom modes 3–6 match an independent parser bit for bit; pinned native records check modes 1/2 and their corrected ×64 wire scaling. Typed configuration rejects incompatible encodings, invalid band/raw shapes, nonpositive raw samples and invalid expanded weights before GPU work. Raw side images use GPU Gradient prediction, signed tokenization and prefix packing under the global MA tree, without transforms or LZ77. A separate input/artifact pair and the existing readback tail share the image job's reservation, submission and completion lifetime. Host validation checks these GPU fragments against caller metadata before HF-global assembly. Independent entropy reconstruction covers all 17 families through positive i32::MAX; seven malformed-fragment variants are rejected. Native comparison corrected the decoder's raw rectangular wire geometry to shorter-axis-horizontal. General transforms retain raw/quantized coefficients on GPU and emit one checked AC fragment per transform per pass, with exact strategy-specific capacities; tiled DCT8 keeps coefficients in 2 KiB workgroup memory and emits one fragment per block per pass. Exact memory plans retain every parameter, resident plane, basis, matrix/order, artifact and readback through validation/cancellation. Tests cover all 162 strategy/correlation/order/matrix streams and fifteen mixed maps under five variants, exact independent permutation decoding, tiled odd/AC/LF boundaries through fragmented 40-byte windows, dense maximum 256x256 fragments, malformed/missing artifacts, independent storage limits, exact/one-byte-deficient budgets and abandoned nonzero work. Existing f64 boundary checks include 2057x2057. Two wide-sample raw images additionally use native libjxl and independent jxl-oxide at the unchanged one-code bound because Rust jxl 0.6.0 disagrees; the corpus records the measured differences. Spectral/quantized AC splitting now supports 1–11 passes, retaining the same quantized GPU coefficients and one submission/map while charging every pass slot. All 27 strategies have exact independent coefficient accumulation and native/Rust/GPU final-image evidence; mixed and tiled cases combine matrices/orders with pass images and ownership checks. Completion requires content-adaptive matrix/order selection, clustering or ANS/LZ policy, broader coefficient conformance and bounded larger-image execution. Stateful entropy must remain on GPU. | `ENT-E01`, `VDCT-E01`, `VDCT-D03` |
...
| `VDCT-E05` | P1 | **Partial** | `VarDctConfig::progressive` selects 1–11 spectral/quantized AC passes for all 27 strategies, mixed maps and tiled DCT8. The GPU splits signed coefficients in canonical spectral coordinates independently of custom order, including refinement after added frequencies; the last pass reconstructs the single-pass coefficients exactly. Pass-major packets carry independent prefix/order metadata. Native intermediate linear images, independent final pixels, bounded fragmented input, retained output and exact admission/cancellation checks cover custom raw/parametric matrices and LF metadata. F32 precision uses pinned scalar libjxl because native SIMD reciprocal estimates exceed the existing bound on a raw-matrix image; ordinary native SIMD still verifies every RGB8 pass at the one-code bound. See the [progressive encoder corpus](CONFORMANCE_CORPUS.md#progressive-vardct-encoding). Multi-pass headers now support all zero-to-four resolution stopping points, with validated factor and wire-index bounds. Raster, integer center-first and caller-explicit AC-group order use bounded host metadata and standard TOC permutations; LF/HF metadata remains before pass-major AC. Independent entropy decoding covers the maximum 65,536-entry generic TOC. Native complete-pass input prefixes match whole-input images and intended resolutions; permuted late corruption preserves earlier validated output. Optional GPU local-contrast saliency now computes exact visible-pixel RGB edge statistics and selects descending mean contrast with deterministic raster ties. Four-word checked records per group share the existing artifact/readback and reservation; the 768/272-byte records reuse former padding, and submission/map counts stay unchanged. Independent edge arithmetic covers clipped/16K axes and all-strategy mixed maps under five variants. A native partial-group example improves the declared intermediate error without changing entropy or final pixels; whole/fragmented pass images, binding limits, malformed records and exact cancellation/admission are covered. Completion requires separate DC progressive frames, broader saliency/perceptual evidence and conformance. | `VDCT-E02`, `VDCT-D06` |
...
| `FRAME-04` | P1 | **Partial** | One-presentation GPU seek/restart preserves original physical IDs, noise seeds, reference-saving color semantics, duration/timecode and finality while suppressing bounded preroll. Required older slot versions can rewind across a later independent anchor; validated target LF/pass images retain ordinary immutable leases. Incremental seek handoff shares the same validated engine and source-token lifetime, including cancellation during preroll and retained updates after completion. Header-only dependency probes and real-GPU mixed/composed/recursive-DC, patch/extra, rejection and lifetime tests cover the current boundary. Non-coalesced frame/layer output, broader skip controls and byte-range seeking remain. [Seek contract](FRAME_SEEKING.md). | `FRAME-03`, `CONT-03` |
| `FRAME-05` | P1 | **Partial** | Both codecs expose shared layered-still/animation sequence sessions; legacy animation names remain aliases. A checked immutable control plan owns timing, signed crops, field presence, reference-only syntax and finality before GPU admission in every scheduling path. Modular Gray/GrayAlpha/RGB/RGBA retains declared alpha association, all five modes, independent color/alpha references and exact RGB/Gray ICC identity with one budgeted header. VarDCT integer/floating Gray/RGB uses XYB or original-sRGB coding through one immutable color plan and single/mixed-transform or tiled backends, configured AC passes, Replace/Add/Multiply and all four post-transform slots; reference-only producers lower to one implicit complete pass for dispatch, packets and admission. Bounded plain `jxli` generation and GPU seeking share the assembled-header dependency plan, including hidden frames and older-reference restart. New layered-still evidence covers ten seven-layer Modular sequences and four VarDCT sequences with cropped/oversized/off-canvas sources, exact physical words, independent native/Rust output or explicit composition, whole/bounded GPU output, retained leases, admission retry and cancellation. Both Rust full-stream oracle limitations are documented without dropping inputs or relaxing bounds. See the [animation](CONFORMANCE_CORPUS.md#vardct-animation-encoding), [reference-only](CONFORMANCE_CORPUS.md#reference-only-frame-encoding) and [layered-still](CONFORMANCE_CORPUS.md#layered-still-encoding) corpora. Six original-RGB layered/animated sequences additionally bind image/frame color metadata to that plan with independent composition and whole/bounded GPU output. Explicit mixed Modular/VarDCT integer/floating Gray/RGB original-sRGB sequences now share image planning, session state and the context budget. Twelve seven-frame still/animation streams cover both coding directions, Prefix/ANS, all three VarDCT backends, independent composition, native/Rust/GPU output, indexed seeking and ownership; see the [mixed-codec corpus](CONFORMANCE_CORPUS.md#mixed-codec-sequence-encoding). The common checked precision now spans 1–31-bit integers with 42 mixed and 12 fixed-codec precision sequences, declared i32 LF buffers and documented 31-bit external whole-stream oracle limitations; see the [integer-input corpus](CONFORMANCE_CORPUS.md#integer-rgb-vardct-input). Floating precision adds 48 mixed and 16 fixed-codec sequences with explicit external whole-stream oracle limitations and native/GPU plus independent physical-frame composition evidence; see the [floating-input corpus](CONFORMANCE_CORPUS.md#floating-rgb-vardct-input). [Gray sequence evidence](CONFORMANCE_CORPUS.md#gray-vardct-and-mixed-input) adds 96 mixed and 64 fixed streams with channel identity, variable physical layouts, indexed references and explicit external-oracle limits. [Enumerated-color sequences](CONFORMANCE_CORPUS.md#enumerated-vardct-and-mixed-input) retain stream-wide color/intent/image white and reject mismatches before advancing either codec. [ICC sequences](CONFORMANCE_CORPUS.md#icc-vardct-and-mixed-input) share exact profile identity and one budgeted header; Original supports cross-codec references, and timed unretained XYB frames enforce the F.2 reference restriction before admission. Shared image orientation and bounded physical-frame names now have [orientation/name evidence](CONFORMANCE_CORPUS.md#encoder-orientation-and-physical-frame-names), including mixed cropped/reference sequences and retained whole/fragmented output. Caller-supplied GPU previews now share image planning, request validation and session-bound completed storage across both codecs and mixed sequences; [preview evidence](CONFORMANCE_CORPUS.md#embedded-preview-encoding) includes independent main animation and indexed seeking. Broader mixed source domains, VarDCT pre-transform references, arbitrary extra-channel blends, automatic thumbnail generation and broader indexing policy remain. | encoder mode items, `CONT-03` |
...
| `IO-02` | P1 | **Partial** | Gray/GrayAlpha/RGB/RGBA buffers now accept packed, planar and split components with bijective RGB/BGR/alpha swizzles, arbitrary integer bit positions, 8/16/24/32-bit storage words and Native/Little/Big byte order. Every 1–31-bit integer depth and all 154 legal floating precisions retain exact source words. Independent unaligned offsets/pitches are validated and rebound per plane; source and artifact limits both split GPU batches. Independent exact/native references, canonical byte equality, whole/bounded GPU output, mixed-layout animations and admission/cancellation tests cover the contract. Enumerated full-range RGB/Gray metadata retains standard/custom primaries/white, seven transfers, four intents and explicit image white. Source alpha association is declared without changing samples, including invisible colors, and final alpha presentation has independent native evidence. Embedded RGB/Gray ICC bytes and original samples now survive both ordinary and explicit device-channel layouts with typed limits, independent/native comparisons and budgeted header ownership. VarDCT and mixed sequences now share this ICC declaration; XYB converts samples through the resident ICC program and Original retains them, with [independent input and ownership evidence](CONFORMANCE_CORPUS.md#icc-vardct-and-mixed-input). Full-resolution same-precision alpha now also accompanies VarDCT and mixed sequences through a shared logical sample plan and GPU Modular side-plane fragments; [alpha evidence](CONFORMANCE_CORPUS.md#alpha-vardct-and-mixed-input) covers words, composition and ownership. [Independent VarDCT scalar inputs](CONFORMANCE_CORPUS.md#independent-vardct-extra-input) now use separate buffers and precision, intrinsic shifts 0–3, named semantics and explicit blend selectors, with union accounting for aliased sources. [Independent Modular/mixed inputs](CONFORMANCE_CORPUS.md#independent-modular-and-mixed-input) now share these contracts. [CMYK ICC input](CONFORMANCE_CORPUS.md#cmyk-encoder-input) now shares the checked primary/scalar plan across Modular, Original/XYB VarDCT and mixed sequences, with integer ink complementation, explicit complemented floats, exact Black/alpha and independent ICC/coefficient/sequence/ownership evidence. [Texture input](CONFORMANCE_CORPUS.md#texture-encoder-input) now shares a checked raw-copy plan across both codecs, mixed sequences and previews: selected uncompressed single-sample 2D color mip/layer bytes remain unchanged, with exact independent words, buffer-equivalent streams, scalar attachments, sampling/references, budget/cancellation and persistent multi-batch ownership evidence. [Explicit YUV input](CONFORMANCE_CORPUS.md#yuv-encoder-input) now uses a checked GPU RGB-F32 preparation across Modular, Original/XYB VarDCT, fixed/mixed sequences and previews, with planar/semi-planar/packed integer layouts, declared chroma/range/matrix/transfer, independent F64/native evidence and shared budget/cancellation ownership. [Separate texture planes](CONFORMANCE_CORPUS.md#texture-encoder-input) now share a checked storage plan for planar/split RGB/CMYK/alpha and explicit YUV conversion across both codecs, mixed sequences and previews, with raw-word/native conformance, exact copy-plus-conversion budgets, aliased-subresource accounting and cancellation evidence. Floating ink-amount conversion, source-domain lossless YUV and native multi-planar texture formats remain; image-domain normalization stays on GPU. | `COLOR-01`, encoder modes |
...
| `API-05` | P1 | **Partial** | Preserve one shared byte budget across encode/decode/display/readback; account scratch, pooled physical bytes, output clones, reference slots, ICC/box buffers, and abandoned futures. Lossless Modular embedded ICC now reserves variable-sized header storage before allocation, retains one header per animation and releases it on completion/error/cancellation independently of active GPU work. Device loss and cancellation must release exactly once. | all allocations |
...
| ID | Pri | State | Requirement and acceptance gate | Depends on |
|---|---:|---|---|---|
| `ENC-01` | P1 | **Partial** | Lossless Modular accepts all 1–31-bit integer and all 154 legal floating Gray/GrayAlpha/RGB/RGBA sources with exact original-word conformance and declared associated/unassociated alpha. Packed/planar/split layouts, swizzles, word byte order and integer bit positions are read directly on GPU. Checked `FloatPrecision` owns sample/exponent bounds and packing validation; source plans, capability negotiation and still/animation headers consume the declared precision. Independent exact/native/GPU tests cover all 154 combinations under Prefix and ANS, RCT/Palette/Squeeze/Weighted composition, exponent mismatch, IEEE byte compatibility and budget/cancellation. Floating samples retain special values and payloads through shared/local trees, animations with per-frame layout changes and bounded streaming. Enumerated RGB/Gray source color, four intents and explicit image white survive still and Replace animation serialization with independent native profile/pixel checks. Alpha composition retains independent reference fields and final output-policy semantics; zero-alpha source colors remain untouched. Embedded RGB/Gray ICC now preserves every original byte with explicit device-channel input, animation identity, size limits and independently verified pixel/profile output. VarDCT accepts all 1–31-bit integer and all 154 legal floating precisions in Gray or packed/planar/split RGB sources in XYB or original-sRGB coding, including bijective swizzles, shared or mixed 8/16/24/32-bit words, sample bit positions and declared byte order. Checked `ColorSampleFormat` supplies logical Gray/RGB channels and precision, masks, GPU normalization and image metadata; one shared source plan and WGSL byte loader own physical addressing for both encoders; mixed sequences bind the same precision for both codecs. Independent coefficient/pixel, saliency, malformed-layout, sequence and ownership checks are recorded in the [integer-input corpus](CONFORMANCE_CORPUS.md#integer-rgb-vardct-input). Floating input has independent coefficient/pixel, sequence, nonfinite rejection, validated completion and ownership evidence in the [floating-input corpus](CONFORMANCE_CORPUS.md#floating-rgb-vardct-input). Color NaN/infinity return typed errors before publication; finite coefficient overflow remains separate. [Source-layout evidence](CONFORMANCE_CORPUS.md#shared-encoder-source-layouts) covers independent pixels, exact layout-equivalent bytes, per-frame layout changes, nonfinite rejection and ownership. [Gray input](CONFORMANCE_CORPUS.md#gray-vardct-and-mixed-input) adds all precisions and 27 strategies, shared Gray/RGB header syntax, aliased GPU working components, 160 fixed/mixed sequences and unchanged numerical/ownership bounds. [Enumerated input](CONFORMANCE_CORPUS.md#enumerated-vardct-and-mixed-input) adds standard/custom RGB primaries and Gray/RGB white, Linear/sRGB/BT.709/PQ/HLG/DCI/Gamma, four intents and exact image white under one checked header/GPU/admission plan. Fixed and mixed sequences preserve the common declaration. [ICC input](CONFORMANCE_CORPUS.md#icc-vardct-and-mixed-input) now preserves RGB/Gray profiles in Original coding and converts them on GPU through relative linear RGB/Gray for XYB. Single/mixed/tiled transforms, all sample precisions, selected LUT/MPE programs, fixed/mixed sequences, prohibited-reference rejection and completion-owned resources have independent evidence. [Alpha input](CONFORMANCE_CORPUS.md#alpha-vardct-and-mixed-input) adds full-resolution GrayAlpha/RGBA at every accepted color precision, declared association, exact GPU Modular words, independent blends/references and completion-owned resources. A shared image-sample plan and one side-plane plan own logical channels, group/pass routing and bounds. Color remains lossy; alpha has exact-word and native/GPU evidence. [Independent VarDCT extras](CONFORMANCE_CORPUS.md#independent-vardct-extra-input) extend the plan to separately declared scalar buffers, all 185 precisions, shifts 0–3 and per-channel blends, with exact pre-resampling words and native/GPU output. [Independent Modular/mixed inputs](CONFORMANCE_CORPUS.md#independent-modular-and-mixed-input) now share scalar declarations, sampling, checked transforms and completion ownership. [CMYK ICC input](CONFORMANCE_CORPUS.md#cmyk-encoder-input) now shares the checked primary/scalar plan across Modular, Original/XYB VarDCT and mixed sequences, with integer ink complementation, explicit complemented floats, exact Black/alpha and independent ICC/coefficient/sequence/ownership evidence. [Texture input](CONFORMANCE_CORPUS.md#texture-encoder-input) now shares a checked raw-copy plan across both codecs, mixed sequences and previews: selected uncompressed single-sample 2D color mip/layer bytes remain unchanged, with exact independent words, buffer-equivalent streams, scalar attachments, sampling/references, budget/cancellation and persistent multi-batch ownership evidence. [Explicit YUV input](CONFORMANCE_CORPUS.md#yuv-encoder-input) now uses a checked GPU RGB-F32 preparation across Modular, Original/XYB VarDCT, fixed/mixed sequences and previews, with planar/semi-planar/packed integer layouts, declared chroma/range/matrix/transfer, independent F64/native evidence and shared budget/cancellation ownership. [Separate texture planes](CONFORMANCE_CORPUS.md#texture-encoder-input) now share a checked storage plan for planar/split RGB/CMYK/alpha and explicit YUV conversion across both codecs, mixed sequences and previews, with raw-word/native conformance, exact copy-plus-conversion budgets, aliased-subresource accounting and cancellation evidence. Floating ink-amount conversion, source-domain lossless YUV and native multi-planar texture formats remain; all input normalization must stay on GPU. | `IO-02`, `COLOR-03` |
| `ENC-02` | P1 | **Partial** | Explicit per-frame color factors 1/2/4/8 now share one checked geometry plan across Modular, VarDCT and mixed sequences. Reduced GPU sources, packed alpha, relative extra sampling, displayed crops/references and coded-grid memory/progression have [independent numerical and ownership evidence](CONFORMANCE_CORPUS.md#per-frame-color-and-relative-extra-sampling). This signals decoder reconstruction; it does not downsample full-resolution sources or establish a quality guarantee. Explicit image orientation and physical-frame names share checked metadata planning with [orientation/name evidence](CONFORMANCE_CORPUS.md#encoder-orientation-and-physical-frame-names); automatic metadata preservation remains separate. Existing explicit codec/progressive/deterministic/memory controls remain bounded profiles. Complete lossless/lossy, distance/quality, effort, decoding-speed, automatic resampling, metadata-preservation and memory/latency policy controls with typed invalid combinations remain. Sequence descriptors now explicitly declare independent GPU preview input, with shared image/frame planning and [validation/ownership evidence](CONFORMANCE_CORPUS.md#embedded-preview-encoding); automatic thumbnail generation remains separate. The common header also emits checked intrinsic display size and all binary16 tone fields without changing coded geometry or GPU resource plans; [independent metadata, display and ownership evidence](CONFORMANCE_CORPUS.md#encoder-intrinsic-size-and-tone-metadata) covers both codecs and mixed/preview sequences. | encoder mode rows |
| `ENC-03` | P2 | **Missing** | Implement automatic Modular versus VarDCT selection and per-feature decisions. Explicit per-frame selection now exists under the common integer/floating Gray/RGB original-component enumerated/ICC-color sequence contract, with optional same-precision alpha, identical forced frame payloads, a common i32 working-buffer declaration and cross-codec composition evidence. This supplies the execution boundary, not an automatic policy. Completion evidence must show the selected path and compare it with forced alternatives. | complete baseline encoders |
| `ENC-04` | P1 | **Partial** | VarDCT accepts up to 256 resolved lossless scalar planes, including packed alpha, with independently checked integer/floating precision, intrinsic dimension shifts 0–3, Alpha/Depth/SpotColor/SelectionMask/Black/CFA/Thermal/Optional metadata and bounded UTF-8 names. One plan owns global-prefix/LF/pass routing, source windows, artifact identities and memory bounds. Still/sequence APIs preserve source samples, support each extra blend/reference and explicit alpha selectors, and reject mismatched input before admission. [Independent-word, native/GPU, metadata and ownership evidence](CONFORMANCE_CORPUS.md#independent-vardct-extra-input) covers this subset. Checked per-frame scalar factors 1/2/4/8 combine with intrinsic shifts for effective sampling through 64. One frame plan supplies source extents, routing and wire factors; reduced inputs stay exact, packed alpha matches the color factor, and independent effective factors cannot be below color. [Shared color/extra sampling](CONFORMANCE_CORPUS.md#per-frame-color-and-relative-extra-sampling) covers 1/2/4/8 color factors across both codecs and mixed frames, with routing relative to the coded grid. Invalid requests preserve admission/finality. [Sampling evidence](CONFORMANCE_CORPUS.md#per-frame-vardct-extra-sampling) includes physical words, unchanged independent F64/native/GPU bounds, changing factors across cropped/reference frames, all blend modes and budget/cancellation. Native libjxl coverage remains limited to effective factors through eight. [Modular/mixed inputs](CONFORMANCE_CORPUS.md#independent-modular-and-mixed-input) now use these declarations through checked Global/LF/Pass plans, both entropy modes, local transforms, composition and cancellation. [Primary CMYK Black](CONFORMANCE_CORPUS.md#cmyk-encoder-input) now shares this routing and ownership plan, following packed alpha before attachments. It retains color-rate sampling and independent references; duplicate Black declarations reject before admission. Per-channel lossy distance and broader invisible-color/semantic conformance remain. Independent VarDCT preview sources also have exact 13-bit depth/non-leading 7-bit alpha and native RGBA evidence in the [preview corpus](CONFORMANCE_CORPUS.md#embedded-preview-encoding). | `ENC-01`, `COLOR-03` |
...
path to adaptive matrices/orders/entropy and progression, while `VDCT-E04` must select
...
Encoders now declare minimum and absolute/relative protected light through the same checked image options. Six freshly encoded Modular/VarDCT F32+alpha streams exercise the requested GPU display curve against native linear samples and independent F64 bounds; [encoder metadata evidence](CONFORMANCE_CORPUS.md#encoder-intrinsic-size-and-tone-metadata) also covers exact binary16 boundaries and shared admission.
