...
| Decoder frame sequence | Bounded image/frame inventories plus `FrameExecutionPlan` describe physical nodes, four versioned reference slots, and presentation ranges. Only one upcoming presentation's entropy/scratch plan is prepared; per-frame GPU ABI and allocations remain those of its Modular/VarDCT producer. | Prefetch admits each presentation through the shared GPU byte budget and frame limiter. A rejected admission retains the exact current producer, including progressive-DC roots. Compressed spans stay under one shared input permit until the last source-dependent submission; cancelling the sequence drops unsubmitted sources immediately. Submitted callbacks and output lease clones retain their physical reservations through completion/drop. Late staged VarDCT submission counts share the producer's atomic counter. | No GPU reference-canvas storage or composition exists yet. Full Replace permits overwritten zero-duration image work to be pruned after bounded structural inventory; skipped entropy is not validated. Pipeline/driver and host descriptor capacity exclusions remain unchanged. |
