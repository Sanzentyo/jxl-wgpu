...
| `vardct_encoder/transforms` | source/params and coefficient/LF/matrix-order/40-byte-task RO, XYB/quantized/artifact RW; each entry point binds its live buffers | 64x1; AC serializer 1x1 | Tier A linear math / Tier B fixed entropy | ordered GPU normalization, shared forward pipeline, AC/LF quantization, one natural-order AC fragment per transform and common LF serializer; complete resident allocation admitted before submission |
| `vardct_encoder/tiled::quantize_blocks` | source/params RO, artifact RW | 64x1 | Tier C (`KernelVariant` linear) | one 2-D workgroup per 8x8 block; checked block-grid/source/artifact ranges; 2,048 shared bytes; tiled DCT8 emits DC plus disjoint word-aligned AC fragments |
...
| encoder `vardct_encode_quantize` | 64-element XYB `vec3<f32>` and AC `vec3<i32>` arrays | 2,048 | selected variant and bytes checked before pipeline creation; quantized AC never enters a storage/readback array |
...
| VarDCT encoder | `VarDctMemoryPlan` charges a 512-byte parameter, runtime artifact and equal readback. LF sections have a 64-word header, two words per LF group, then independently 64-word-aligned strategy (`N`), DC/token/extra (`3N` each), and checked entropy sections. Single transforms add one AC length and a slot for three counts plus up to `area-area/64` coefficients per channel; the largest slot is 127,010 words. `transform` adds XYB/raw/quantized planes (`12*area` each), LF (`12*area/64`), matrix/order (`16*area` per distinct strategy), 40-byte transform tasks and the forward primitive's exact transient bytes. Tiled DCT8 uses no resident transform storage and adds `N` AC lengths plus `125N` fragment words. All sections align to 256 bytes. | The shared budget retains every encoder-owned buffer through validation or abandoned completion; the source binding remains caller-owned. DCT8 single-transform ownership is exactly 10,384 bytes. Validators check section metadata, DC residuals/histogram, AC counts/ranges, exact bit consumption and zero padding. Only complete compressed fragments cross the readback boundary for AC; host assembly preserves their bits. Binding, buffer, workgroup and dispatch limits are checked before submission. | All 27 strategies emit real AC singly and in validated mixed maps, including non-DCT8 replicated edges. Mixed maps use exact per-strategy AC slot capacities and batch dispatch with shared image-wide arenas. Adaptive entropy, content-based strategy search and rate control remain incomplete. Contextual/ANS policies must preserve entropy state on GPU. Actual-adapter tests cover all 27 strategies, five linear variants, 2057x2057 tiled AC and 16K panoramas, exact/one-byte-deficient budgets, abandoned nonzero work, malformed artifacts, and independent source/artifact/metadata binding limits. |
