...
768 bytes and transform tasks remain 44 bytes; one DCT8 transform owns 12,180 bytes in total.
...
four storage bindings and keeps its 2,052 workgroup bytes. Matrices/orders do not add a GPU submission
...
768 bytes, the main artifact header remains 272 bytes, and matrix/order entries remain 24 bytes.
...
the source word width (1/2/4 bytes) and valid mask (`2^bits - 1`, including all 32 bits for
binary32). Word 188 is the floating exponent width, zero for integers; words 189–190 are
the general floating-source validation offset/count. Word 191 and unused pass descriptors
are zero. The immutable color/sample plan supplies these fields, source binding geometry,
image bit depth/exponent and HF multipliers. Both paths share byte-addressed word loading
and padding masking. Float conversion checks raw nonfinite exponents and rebases finite
fields into F32; only integers divide by the sample mask. Original RGB skips color conversion.
Rows contain `width * 3 * word_bytes`; existing last-word, alignment and u32 address checks
cover arbitrary byte offsets and padded rows. The public `xyb_bytes` subtotal accounts
for normalized three-plane storage in either domain.

General floating normalization adds artifact binding 2 to its existing source/parameter/
