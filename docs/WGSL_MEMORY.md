...
Its 288-byte uniform at binding 4 retains output/input dimensions at offsets 0/8, source strides
...
admission charge their full 448 bytes. Read-only storage binding 6 supplies optional signed i32
...
Final packing uses either the shared 288-byte `ImageOutputParams` with planar source overrides or
...
The common packer selects one 288-byte parameter set for the actual original or linear-original
...
The shared `ImageOutputParams` is 288 bytes: `alpha[0]` at byte 176 selects Preserve (0),
...
into the existing aligned all-channel render arena; its 288/160-byte uniforms and each 80-byte
Gaborish/EPF or 48-byte upsampling uniform are included exactly once. A 37×17, factor-2 color plan
with Gaborish and two EPF passes accounts 11,652 working-plane bytes and 912 uniform bytes,
...
an 80-byte normalization uniform and 448 bytes for packing. The tested 256-byte alignment makes
that total increase 158,472 bytes. One byte below the required capacity leaves no partial permit,
...
three 48-byte interpolation uniforms per level, and the color packer's 448-byte uniforms. The
...
`ImageOutputParams` is 288 bytes; `transfer_parameters` begins at byte 192. Shared image selectors
...
reserves no color-packing uniforms; color previews retain the 448-byte conversion plan. Both
...
the 320-byte ICC dispatch storage, optional four-byte validation map, the 288-byte output-packing
...
Same-profile device output without requested tone mapping allocates no ICC program or conversion intermediate. The 288-byte
...
`ImageOutputParams` is now 288 bytes. Its prior fields keep their offsets; `transfer_parameters.w`
...
The 288-byte image-output record appends `ToneMappingParams` at byte 240 without moving earlier
...
uniforms total 448 bytes. ICC opcode 11 embeds the same 48-byte payload and a 16-byte stage entry;
