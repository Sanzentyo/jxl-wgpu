...
| `vardct_packet` (decoder) | codestream/MA metadata RO, reconstruction/raw metadata/coefficients/status RW, control U, Modular params RW | 1x1 | Tier B (fixed) | combined global-tree and split LF/HF local-tree entry points use logical channel widths with explicit physical strides; the split path preserves LF storage and maps only the bounded aggregate cursor/status records between GPU stages |
...
| decoder `vardct_pass_group` | codestream/entropy bundle RO, quantized-LF plus disjoint LZ slices/status RW, 112-byte pass params RO; artifact/order RO, coefficients RW, sink U | 1x1 | Tier B (fixed) | one sequential invocation per pass group; eight storage bindings meet the portable stage limit; the 48-byte block-context table ABI addresses QF and signed X/Y/B LF thresholds; exact token bounds, task geometry, order coordinates, coefficient capacity, and status range are checked |
...
`ShaderParams` storage record and the 208-byte VarDCT packet parameter record. Consumers supply
...
the overshoot in the next upload. The lane tail is rounded to 16 bytes and stores eight `u32`
values (32 bytes): bit cursor, ANS state, LZ copy remaining/position, entropy decoded count, last
value, consumer decoded count, and sticky error. The descriptor-sized LZ history ring remains in
the preceding scratch region and is not duplicated per segment.
...
| Bounded VarDCT decoder | `VarDctDecodeMemoryStats` accounts codestream, entropy metadata including HF block-context thresholds, every LF group's parameters/LZ/status, the 39-descriptor/all-order coordinate table, packet/artifact records, occupancy, reconstruction, capacity-strided raw metadata, coefficients, shared full-image LF/correlation/default-matrix/AFV resources, XYB/transform/output storage, one optional three-plane restoration scratch set, the 80-byte Gaborish uniform, full-image EPF inverse-sigma plane, one 80-byte sigma uniform per LF group, one 80-byte filter uniform per iteration, and aggregate validation staging. Each LF group's quantized LF and HF LZ bytes are logical slices of its own physical reconstruction allocation. `deferred_hf_modular_metadata` identifies local-tree plans whose exact HF metadata size is cursor-dependent. | One shared backend byte reservation covers all LF-group transient buffers until final packet/artifact/pass-group validation, and output bytes until the last `GpuBufferLease` clone is dropped. Global-tree frames reserve their metadata up front and use one submission/map. Local-tree frames initially reserve all LF metadata, then admit only a larger HF metadata peak's byte difference through the same budget before their second submission; each stage uses one aggregate map. Cancellation leaves callback-owned permits and buffers alive until mapping completes. Adaptive LF smoothing runs once over the shared atlas; the standard skip flag uses a GPU buffer copy. Gaborish and every EPF pass reuse the same scratch planes through a restoration cursor rather than reserving one image set per stage. Device binding limits are checked per physical LF-group buffer while budget statistics retain exact cross-group totals. | Pipeline/driver-private allocations are excluded. The host parses cursor-dependent HF descriptors between submissions but never decodes image entropy or owns an intermediate image. |
