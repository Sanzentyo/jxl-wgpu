...
| `jxl_wgpu/forward_vardct.wgsl` | `forward_vardct::Params` / `Params` | six transform/LF geometry words, reserved word, linearized workgroup X count, three vec4 records for strides, scalar origins and constant-basis offsets | 80 | 16 | uniform |
...
| `jxl_wgpu_encode/vardct_encoder/single.wgsl` | four host words / `QuantizationEntry` | three f32 dequantization scales followed by one u32 natural-order position | 16 | Rust 4 / WGSL 16 | read-only storage element |
...
| `forward_vardct` | three source planes and basis RO, horizontal/coefficient/LF RW, U; each entry point binds only its live buffers | 64x1 | Tier A (`KernelVariant` linear) | regular horizontal/vertical DCT or special constant-basis transform, followed by normative LF extraction; 2-D linearized dispatch, zero shared memory, typed geometry/range/alias/storage validation |
| `vardct_encoder/single` | source/params and coefficient/LF/matrix-order RO, XYB/quantized/artifact RW; each entry point binds its live buffers | 64x1; AC serializer 1x1 | Tier A linear math / Tier B fixed entropy | ordered GPU normalization, shared forward pipeline, AC/LF quantization, single natural-order AC fragment and common LF serializer; complete resident allocation admitted before submission |
...
| VarDCT forward primitive | `ForwardVarDctMemoryPlan`: 80-byte uniform; regular basis `4*(w*w+h*h+(w/8)^2+(h/8)^2)` and `12*w*h` horizontal bytes, or 16,384-byte special basis with no horizontal buffer. Caller-owned coefficients occupy `12*w*h` bytes and LF `12*(w/8)*(h/8)`. | `ForwardVarDctScratch` exposes and retains all three temporary allocations; callers include them in their completion lease. Binding sizes/usage, independent strides/origins, writable overlap, WGSL-u32 addresses and device/dispatch limits are checked. | Basis construction expands bounded strategy constants only. All image-dependent arithmetic runs on GPU. Three source planes may alias each other, but output and scratch writes cannot overlap inputs or each other. |
| VarDCT encoder | `VarDctMemoryPlan` charges a 512-byte parameter, runtime artifact and equal readback. LF sections have a 64-word header, two words per LF group, then independently 64-word-aligned strategy (`N`), DC/token/extra (`3N` each), and checked entropy sections. Single transforms add one AC length and a slot for three counts plus up to `area-area/64` coefficients per channel; the largest slot is 127,010 words. `transform` adds XYB/raw/quantized planes (`12*area` each), LF (`12*area/64`), matrix/order (`16*area`) and the forward primitive's exact transient bytes. Tiled DCT8 uses no resident transform storage and adds `N` AC lengths plus `125N` fragment words. All sections align to 256 bytes. | The shared budget retains every encoder-owned buffer through validation or abandoned completion; the source binding remains caller-owned. DCT8 single-transform ownership is exactly 10,340 bytes. Validators check section metadata, DC residuals/histogram, AC counts/ranges, exact bit consumption and zero padding. Only complete compressed fragments cross the readback boundary for AC; host assembly preserves their bits. Binding, buffer, workgroup and dispatch limits are checked before submission. | All 27 single transforms emit real AC, but mixed maps, non-DCT8 edges, adaptive entropy and rate control remain incomplete. Contextual/ANS policies must preserve entropy state on GPU. Actual-adapter tests cover all 27 strategies, five linear variants, 2057x2057 tiled AC and 16K panoramas, exact/one-byte-deficient budgets, abandoned nonzero work, malformed artifacts, and independent source/artifact/metadata binding limits. |
