...
AC use the same checked overlap/batching values without sharing their WGSL state layout; VarDCT LF/HF
packet adoption remains unfinished. Each split segment is four-byte
...
| Bounded VarDCT decoder | `VarDctDecodeMemoryStats` accounts codestream, entropy metadata including HF block-context thresholds, every LF group's parameters/LZ/status plus 464 bytes of AC resume state per pass group, the reusable AC stream/parameter peaks and batch count, the 39-descriptor/all-order coordinate table, packet/artifact records, occupancy, reconstruction, capacity-strided raw metadata, coefficients, shared full-image LF/correlation/default-matrix/AFV resources, XYB/transform/output storage, one optional three-plane restoration scratch set, restoration uniforms, and aggregate validation staging. | One shared backend byte reservation covers all LF-group transient buffers until final validation and output bytes until the last lease clone. Whole-range global-tree frames retain one submission/map. Windowed AC inserts ordered submissions between pre-coefficient lowering and the resident render tail, rewrites one stream/parameter pair only after the previous queue use, and maps status once after the final window. Local-tree frames first map LF cursors; cancellation leaves callback-owned permits and buffers alive until mapping completes. Gaborish and EPF share one ping-pong scratch set. | Packet LF/HF entropy still binds whole ranges, and the AC cap is not yet reduced dynamically by the byte budget. Pipeline/driver-private allocations are excluded. The host parses cursor-dependent HF descriptors but never decodes image entropy or owns an intermediate image. |
