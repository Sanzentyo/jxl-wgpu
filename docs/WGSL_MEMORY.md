...
| `jxl_wgpu_decode/vardct_epf.wgsl` | `EpfSigmaUniform` / `Params` | block/task/sharpness geometry, artifact status/task offsets, global scale, quant multiplier, two four-value sharpness LUT rows | 64 | 16 | uniform |
...
| `jxl_wgpu_decode/vardct_artifact.wgsl` | `HfMetadataLoweringParams` / `Params` | six `vec4<u32>` records for dimensions/capacities/image/artifact/metadata/source offsets, then seven `vec4<u32>` records containing all 27 strategy matrix offsets | 208 | 16 | uniform |
...
| `vardct_packet` (decoder) | codestream/MA metadata RO, reconstruction/raw metadata/coefficients/status RW, control U, Modular params RW | 1x1 | Tier B (fixed) | sequential bounded LF and HF-metadata streams use logical channel widths with explicit physical strides; all token, reconstruction, raw-metadata, coefficient, and status ranges are checked |
| `vardct_artifact` (decoder) | raw metadata RO, artifact/occupancy/resources RW, U | 1x1 | Tier B (fixed) | validates non-overlapping mixed varblocks, compacts all 27 strategy buckets, and emits three bounded indirect records per strategy plus exact coefficient ranges |
...
| `vardct_epf_sigma` (decoder) | raw metadata/artifact RO, inverse-sigma RW, U | 64x1 | Tier A (`KernelVariant` 1-D) | one invocation per validated transform task; artifact status/task count gate writes, while block extent and sharpness are bounded before addressing |
...
| Bounded VarDCT decoder | `VarDctDecodeMemoryStats` accounts codestream, both entropy metadata bundles including HF block-context thresholds, per-group parameters/LZ/status, the 39-descriptor/all-order coordinate table, packet/artifact records, occupancy, reconstruction, capacity-strided raw metadata, coefficients, LF/all-default-matrix/AFV resources, XYB/transform/output storage, one optional three-plane restoration scratch set, the 80-byte Gaborish uniform, EPF inverse-sigma plane, 64-byte sigma uniform, one 80-byte filter uniform per iteration, and the aggregate validation staging map. Quantized LF and HF LZ bytes are separately reported logical slices of one physical reconstruction allocation. | One shared backend byte reservation covers transient buffers until status validation and output bytes until the last `GpuBufferLease` clone is dropped. Gaborish and every EPF pass reuse the same scratch planes through a restoration cursor rather than reserving one image set per stage. | Pipeline/driver-private allocations are excluded; the current one-LF-group cap bounds the largest accepted image. |
