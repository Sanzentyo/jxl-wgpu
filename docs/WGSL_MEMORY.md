...
| `jxl_wgpu_encode/vardct_encoder/transforms.wgsl` | `TransformTask` | block origin, coefficient/LF offsets, transform extent, matrix/order offset, AC slot offset/capacity, strategy ID | 40 | 4 | read-only storage element |
| `jxl_wgpu_encode/vardct_encoder/control.wgsl` | `VarDctKernelParams` / `Params` | source/block geometry, LF/control section ranges, topology/LF grid, LF quantization/correlation, separate 19-entry DC/HF prefix tables, HF correlation/quantization, five AC descriptor/fragment fields, linearized workgroup X count, 15 pads | 512 | 4 | read-only storage |
...
## VarDCT encoder coefficient orders
...
Tiled DCT8 reads a 64-entry, 12-byte-stride X/Y/B index table at storage binding 3. Its 768 bytes
are reported as `coefficient_order_bytes`, included in exact admission before allocation, and
held by the map-callback lifetime alongside the artifact and readback. The tiled pass requires
four storage bindings and keeps its 2,052 workgroup bytes. Orders do not add a GPU submission
