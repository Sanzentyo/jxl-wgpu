...
- Modular encoding, entropy reconstruction and Palette share `jxl_wgpu/shaders/modular_predict.wgsl`. Its signed 64-bit predictions,
  averages, corrections, and weighted products use the low/high `u32` pair in
  `jxl_wgpu/shaders/modular_int64.wgsl`; those temporaries add no storage binding or allocation. Committed true
...
- The Modular inverse planner uses one word-addressed arena rather than one buffer per intermediate
  ...
  entropy words and a 90-word peak. Empty-residual inverses retain the average span and restore
  its geometry/shifts without an allocation, uniform or copy. Consecutive Squeeze dispatches share
  one compute pass and retain their individual uniforms and ordered arena accesses. An
  RCT/Squeeze/RCT plan preserves five-job order over three
...
| `jxl_wgpu_encode/lossless_modular.wgsl` | `ModularParams` / `Params` | `width, height, output_word_offset, channel, channels, sample_mask, rct_type, big_endian`; four 24-byte source records (`row_stride, byte_offset, pixel_stride, word_bytes, bit_shift, plane`); predictor and WP scratch offset, seven coefficients, four maximum weights, LZ77 mode/scratch offset/hash mask; Squeeze mode/source width/source height; Palette capacity/scratch offset/hash mask; group channel count; Palette delta predictor/delta capacity/implicit depth/component begin/component count; resolved sample source/Squeeze band; transform program/sample word offsets | 256 | 4 | read-only storage element |
...
- Codestream storage and the decoder prefix lookup are variable arrays, not fixed records. The
  ...
  `GpuDecoder::stream` admits each nonempty transport slice under one immutable shared token.
  A complete preview prefix clones only intersecting slices and their tokens; later main slices
  never become preview-owned. A clipped final slice still retains its original range's entire
  charge because the underlying allocation remains shared. Byte and span limits are reserved
  together before scanner mutation and released together by the token's final owner. The source
  follows its last source-using submission or cancellation lifetime. This host-retention budget
  is deliberately distinct from the GPU allocation budget because source and destination coexist
  while an upload is populated. Prefix span tables retain original offsets and expose only the
  complete preview range; no entropy rewrite, host concatenation or GPU ABI change is involved.
...
| `forward_vardct` | three source planes, task descriptors and basis RO, horizontal/coefficient/LF RW, U; each entry point binds only its live buffers | 64x1 | Tier A (`KernelVariant` linear) | regular horizontal/vertical DCT or special constant-basis transform, followed by normative LF extraction; 2-D linearized dispatch, zero shared memory, typed geometry/range/alias/storage validation; 20-byte task descriptors select independent source origins and disjoint coefficient/LF ranges |
...
- Encoder artifact storage and mapped readback have identical checked, 4-byte-aligned sizes. The
  array of 256-byte `ModularParams` records and artifact/readback buffers are leased as one exact-size set, so a
  ...
  Each channel's 400-byte header and 16-byte events are parsed as checked `Pod` records. Decoder codestream
  ...
  browser continuations. RCT selection adds no image allocation, submission, binding or readback,
  and leaves parameter stride and artifact sizes unchanged. Source-window limits may split additional batches,
  ...
  256-byte parameter record and six bindings. Per-channel event artifact capacity is
  `400 + 16 * (width * height + ceil(width * height / 8) + 1)` bytes plus optional Weighted/LZ77
  scratch, evaluated for the actual edge/crop group. LF geometry scales by eight per axis. Larger
  groups must individually fit source/artifact limits; batching never splits the channel set of a group. Header geometry,
  ...
  each group's first artifact adds `4 * (2 + k * selected_components + next_power_of_two(2*k))`
  bytes for total/delta counts, a channel-major dictionary and at-most-half-full hash table. The whole
  artifact allocation is cleared before dispatch. Only the first invocation builds that group's
  dictionary and encodes all its channels; other invocations return, so no cross-invocation
  dependency or additional dispatch is needed. The meta channel reserves width `k` and height
  `selected_components`; tokenization uses the actual GPU count. The index replaces the selected
  range and remaining image components retain their order. Optional Squeeze starts after the meta
  channel and transforms every image channel and its bands. The encoded channel count is
  `1 + (source_components - selected_components + 1) * 2^stages`, at most 17. Weighted/LZ77
  capacities use the reserved dimensions. `palette_scratch_bytes` reports
  peak dictionary/hash storage already included in artifact/readback ownership and admission.
  Mapped private scratch is not interpreted as pixels by the host: it reads only the counts,
  validates total `1..=k`, delta `0..=total` and both partition limits, then verifies exact
  dynamic table coverage and serializes encoded events. Event-count
  sentinels `0xfffffffd` and `0xfffffffc` respectively reject capacity overflow with
  `BackendError::ModularPaletteOverflow` and invalid lookup with `BackendError::InvalidArtifact`.
  ...
  Delta entries have a limit of 66,816; delta and mixed modes append `group_pixels * selected_components` raw residual
  words after the hash table, followed by five words per source column only for Weighted delta
  prediction. The first invocation scans each selected original post-RCT component with the chosen
  ...
  artifact/readback admission. Host code does not inspect residual/dictionary scratch.
  Dictionary storage reserves the delta partition before the color partition, while token reads
  compact the actual used entries into delta-then-color wire order. Hash comparisons include the
  partition, and absolute matches take priority over residuals. Both used counts are published
  ...
  Implicit entry component indices are relative to the selected range. Unselected source words
  pass through the ordinary post-RCT path before optional Squeeze, including its unchanged
  signed-overflow rejection. Streamed admission checks the reported peak before the first
  worker/batch allocation; each batch still reserves its actual bytes through completion and
  mapped consumption, so subsequent concurrent pressure remains a typed failure.
...
| VarDCT forward primitive | `ForwardVarDctMemoryPlan`: 64-byte uniform and 20-byte descriptor per task, with one basis/uniform per strategy batch; regular basis `4*(w*w+h*h+(w/8)^2+(h/8)^2)` and `12*w*h*task_count` horizontal bytes, or 16,384-byte special basis with no horizontal buffer. Each task addresses `12*w*h` coefficient bytes and `12*(w/8)*(h/8)` LF bytes in caller-owned output arenas. | `ForwardVarDctScratch` exposes and retains all parameter, task, basis and horizontal allocations; callers include them in their completion lease. Binding sizes/usage, independent strides/origins, writable overlap, WGSL-u32 addresses and device/dispatch limits are checked. | Basis construction expands bounded strategy constants only. All image-dependent arithmetic runs on GPU. Three source planes may alias each other, but output and scratch writes cannot overlap inputs or each other. |
| VarDCT encoder | `VarDctMemoryPlan` charges an 916-byte parameter, runtime artifact and equal readback. LF sections have a 68-word header followed by two words per LF group, then independently 64-word-aligned strategy (`N`), DC/token/extra (`3N` each), and checked entropy sections. Single transforms add one AC length and a slot for three counts plus up to `area-area/64` coefficients per channel; the largest slot is 217,730 words. `transform` adds normalized-color/raw/quantized planes (`12*area` each), LF (`12*area/64`), matrix/order (`24*area` per distinct strategy), 44-byte transform tasks and the forward primitive's exact transient bytes. Tiled DCT8 retains a 1,536-byte matrix/order table, uses no resident transform storage and adds `P*N` AC lengths plus `P*214N` fragment words for `P` passes. Single/mixed transform length arrays and slot arenas are also repeated per pass; resident coefficients and matrix/order metadata are shared. Strategy, sample and entropy sections align to 256 bytes. General floating sources append aligned per-normalization-workgroup status records, charged in artifact and readback and exposed as `source_validation_bytes`; integer/tiled paths add none. Optional raw matrices add bounded sample/descriptor/prefix input, compressed artifact/status and an equal-sized tail in the existing readback. | The shared budget retains every encoder-owned buffer through validation or abandoned completion; the source binding remains caller-owned. Default RGB8 DCT8 single-transform ownership is exactly 12,328 bytes, including per-channel order indices. GPU quantizers flag signed-32-bit overflow before publishing an artifact. Validators check section metadata, DC residuals/histogram, AC counts, exact bit consumption and zero padding; both entropy alphabets cover all 32 bits. Only complete compressed fragments cross the readback boundary for AC; host assembly preserves their bits. Binding, buffer, workgroup and dispatch limits are checked before submission. | All 27 strategies emit real AC singly and in validated mixed maps, including non-DCT8 replicated edges. Mixed maps use exact per-strategy AC slot capacities and batch dispatch with shared image-wide arenas. Adaptive entropy, content-based strategy search and rate control remain incomplete. Contextual/ANS policies must preserve entropy state on GPU. Actual-adapter tests cover all 27 strategies, five linear variants, 2057x2057 tiled AC and 16K panoramas, exact/one-byte-deficient budgets, abandoned nonzero work, malformed artifacts, and independent source/artifact/metadata binding limits. |
| Lossless Modular encoder | `LosslessModularMemoryPlan` reports source binding ranges (full and peak), 256-byte-aligned parameter storage, peak artifact storage including its Weighted row-state and LZ77 scratch subtotals, mapped readback, diagnostic total artifact bytes, batch count, exact GPU submission count, streaming mode, valid bits, exponent width (zero for integers), component storage bytes, channel count, format, group grid, owned bytes/job and addressed bytes/job. `EncoderBufferPoolStats` separately reports exact idle bytes, three-buffer set counts, hits, misses and evictions. | Resident GPU jobs synchronously reserve their parameter/artifact/readback bytes from the shared `MemoryBudget`; the complete encoder separately retains any ICC header reservation described below. Streamed jobs instead reserve each live batch's parameter/artifact/readback bytes before leasing buffers; native worker admission failures are delivered by completion. The exclusive buffer lease and permit survive until mapped artifacts are consumed. If the future is abandoned, its callback-owned lifetime unmaps and returns the set only after mapping resolves; the mapped artifact buffer is parsed in place instead of being duplicated into a host `Vec`. A bounded poll slot is reserved before `Queue::submit`, so poll saturation returns both memory and buffers without orphaning GPU work. The idle pool uses exact artifact-size matching and has an independent 32 MiB default hard limit, configurable down to zero, plus a 256-set object-count cap for tiny workloads. | Caller-owned source bindings are sampled directly: they are reported as addressed, are neither copied nor pooled, and are not charged as encoder-owned. Queue/driver-private command metadata excluded. Physical caller-visible allocation is bounded by live admitted bytes plus the separately reported idle-pool bytes. |
...
Embedded RGB/Gray ICC encoding adds host metadata ownership to the Modular budget without changing
WGSL records, bindings or GPU submission counts. `icc_profile_bytes` reports retained caller-owned
original bytes as addressed memory. The complete encoder's `icc_storage_bytes` is twice the exact
image-header size: the live header and a possible temporary copy during final resizing/wrapping.
It is included in owned/addressed job totals and reserved before writing variable-sized ICC data.
The writer streams predicted header bytes and literal profile data into that one allocation, using
no intermediate transformed-profile vector. Final assembly moves the header allocation and reserves
its extension once. The returned codestream/container vector becomes caller-owned on completion.
Still futures release the header permit at `Ready`, including errors, even if the future object
remains alive. Dropping an unfinished future retires its host header; active GPU batches continue
to own their independent source leases and permits through completion. An animation admits one
header at session creation, keeps it across all frames and retires it on finish/error/drop. Its
frame backend reports zero ICC storage to avoid charging the image header per frame. Original and
...
Its 304-byte uniform at binding 4 retains output/input dimensions at offsets 0/8, source strides
...
admission charge their full 464 bytes. Read-only storage binding 6 supplies optional signed i32
...
## VarDCT encoder matrices and coefficient orders

General-transform quantization binding 7 uses 24-byte entries: `array<f32, 3>` dequantization
scales at byte 0 and `array<u32, 3>` X/Y/B coefficient indices at byte 12, both with four-byte
scalar stride. Each caller-selected order is a validated permutation with a fixed LF prefix.
The parallel quantizer visits every AC location exactly once using X's permutation and writes all
three channels; the scalar GPU serializer follows each channel's own permutation. Entries are
shared by same-strategy tasks and charged in `quantization_metadata_bytes`. Parameters remain
916 bytes and transform tasks remain 44 bytes; one DCT8 transform owns 12,328 bytes in total.
Default and caller-selected parametric/raw modes 0–7 share this layout. The host expands bounded
matrix metadata for the 17 families, multiplying raw samples by their denominator; the GPU
performs all image-dependent quantization. Raw side-image entropy has separate storage below.

Tiled DCT8 reads a 64-entry matrix/order table with the same 24-byte stride at storage binding 3.
Its 1,536 bytes are reported as `VarDctMemoryPlan::quantization_metadata_bytes`, included in exact admission before allocation, and
held by the map-callback lifetime alongside the artifact and readback. The tiled pass requires
seven storage bindings and keeps its 2,052 workgroup bytes. Matrices/orders do not add a GPU submission
or an image-coefficient readback. Cancellation cannot release the table before completion.

The decoder's HF-order metadata cursor derives its symbol bound from the declared family mask:
`sum(3 * (1 + area - area/64))`, at most 387,867 for all thirteen families. This replaces the
unrelated MA-tree default only for this finite control grammar. Entropy-descriptor limits,
permutation length/rank checks, final-state validation and packet-end bounds remain unchanged.

## Raw VarDCT matrix encoding

Caller-selected mode-7 matrices are immutable positive signed-32-bit X/Y/B samples plus a
positive finite binary16 denominator. The wire raster has its shorter axis horizontal; its
flattened samples already match the canonical matrix entries used by transposed strategies.
The host validates exact family sizes and dequantization products in `(0, 1e8)` before planning.
There are at most 17 raw tasks, each with at most 65,536 samples per channel.

`raw_matrices.wgsl` uses two storage bindings and no uniforms or shared memory. Input word 0
is the task count; words 1–66 hold 33 prefix `(bits, length)` pairs. Five-word tasks start at
word 67, followed by planar X/Y/B samples. For `N` raw families with areas `A_i`, input size is
`4 * (67 + 5*N + 3*sum(A_i))` bytes. Each output starts with `N` four-word completion records,
followed by disjoint word-aligned entropy slots. With `B = max(prefix_length[s] + max(s-1, 0))`,
the artifact size is `4 * (4*N + sum(ceil(3*A_i*B/32)))` bytes. Planning checks sample/fragment
address arithmetic and device storage, buffer and dispatch limits before allocation.

One invocation owns each family's output words and performs clamped-Gradient prediction,
signed hybrid-uint tokenization and prefix packing. The artifact is cleared before dispatch;
each append checks capacity and writes at most two words. Raw work joins the image's command
buffer and copies into the tail of its existing readback. No additional queue submission or
map is introduced. `raw_matrix_input_bytes` and `raw_matrix_artifact_bytes` are explicit memory
subtotals; `readback_bytes` includes the raw tail. Exact admission charges all three additions,
and the job callback retains both buffers and their permit through completion or cancellation.

Mapped validation requires the ready marker, task identity, exact sample count and bit length,
checks every prefix/extra against the bounded caller metadata, and rejects nonzero trailing
padding. Only then can frame assembly append the GPU fragment after its raw Modular header.
Host work neither produces raw entropy nor decodes image coefficients. Image parameters remain
916 bytes, the main artifact header remains 272 bytes, and matrix/order entries remain 24 bytes.
All-family exact-budget, cancellation, independent entropy and corruption tests cover this path.

## Progressive VarDCT encoding

Gray input aliases its source in the parameter record and retains all existing bindings. The immutable
...
image descriptor fixes geometry/timebase; the backend binds Gray/RGB channels and integer/floating precision and original enumerated-color metadata; per-frame coding
...
The shared resident/streamed Modular and resident VarDCT map callbacks release their lifetime references before
...
`VarDctKernelParams` is a 916-byte storage record. Words 168–169 contain AC pass count/word
...
Words 184–185 are sample mask and exponent width (zero for integers); 186–187 hold source-validation offset/count. Word 188 is the big-endian flag. Words 189–212 are
four input-component `Source` records: `row_stride, byte_offset, pixel_stride, word_bytes,
bit_shift, plane`, each with 24-byte stride and four-byte alignment. Words 213–228 hold one
64-byte `SourceColor` record: transfer selector, gamma, intensity, four luminance/OOTF scalars
and nine row-major primary-matrix scalars. The arrays retain four-byte alignment. The checked
wire-color plan lowers these once, using shared transfer selectors and quantized xy/gamma.
The additional 64 bytes are included exactly once in job admission; no image-sized buffer,
new binding or submission is added. Unused pass descriptors
are zero. Rust POD/offset checks and Naga reflection verify the layout.

`source::SourceLayout` and `SourceWindows` are shared with Modular. Each plane gets an aligned
bounded binding at 0/12/13/14, with unused entries aliasing the first valid plane. The common
`source.wgsl` loader supports packed/planar/split addressing, swizzles and declared word bit/byte
order. The largest window must fit the storage-binding limit and u32 addressing; absolute
...
explicitly complemented inputs leave it clear. `SourceLayout` lowers separate CMY/alpha, Black and
CMYK views, each with at most four bound planes, so five physical CMYKA planes retain the same
binding limits. Scalar passes bind the primary buffer for Black and attached buffers for declared
extras. Range unions and completion ownership account for each shared allocation once.
...
component bindings and writes one completion word per workgroup, after a workgroup barrier.
The aligned section follows AC storage and precedes optional saliency records; its bytes
are exposed in `source_validation_bytes` and included once in each artifact and readback
...
XYB ICC input reuses the same 916-byte ABI in `icc_input.wgsl`, with source bindings 0/12/13/14,
parameters at 1, artifact at 2 and normalized device planes at 3. Its checked pipeline override
`icc_channels` is one for Gray, three for RGB or four for CMYK. Each invocation owns one padded pixel in each
...
writes one linear Gray or three linear BT.709 planes. CMYK uses the resident complemented-input
contract; its four-plane input and three-plane output have separate checked sizes. General normalization preserves the first pass's
...
plus the lowered resident program, original 916-byte layout and 320-byte resident dispatch.
...
storage limits and u32 offsets are checked before admission. Gray's forward source records
...
raw source exponents. The proxy retains the 765-per-edge bound and never feeds forward
transforms. Image headers continue to declare i32 Modular working buffers independently of
source precision.
...
`ac_fragment_offset + pass * ac_pass_words`. Transform word offsets and capacities remain
unchanged within that arena. Tiled DCT8 has `T = ceil(width/8)*ceil(height/8)` and 214 words
per slot; mixed maps retain each transform's exact strategy-specific capacity. Planning
checks every multiply/add before conversion to GPU addresses and rejects storage/buffer
limits before admission. In particular, an eleven-pass 16K square overflows the checked
arena representation and is rejected without allocating it.

The GPU quantizes once, then splits each coefficient in canonical spectral coordinates.
Signed division toward zero and subtraction of prior included contributions support the full
i32 range, including `i32::MIN`, without an absolute-value overflow. Custom coefficient
orders affect serialization only. All 1–11 pass slots are disjoint; coefficients stay in the
existing resident planes or tiled DCT8's unchanged 2,052 workgroup bytes. LF, raw-matrix
fragments, matrix/order metadata and transform scratch are shared across passes.

The original one submission and aggregate map cover every pass. Host validation checks the
pass count, every length, symbol/count range, complete bit consumption and padding before
assembling any frame packet. Tiny multi-pass images use separate LF-global/LF/HF-global and
pass packets; only one-group, one-pass images use the fused packet. The full expanded artifact
and readback remain reserved through validation or abandoned completion. Tests cover corrupt
...
They do not add GPU buffers, change the 916-byte parameters or 272-byte artifact header, or
...
left/up image neighbors through the same checked per-component source bindings. A 256-word workgroup
...
the score. The pass uses six storage bindings and 1,024 workgroup bytes, with no new buffer.
...
events. The raw histogram has 33 bins, including token 32 with 31 extra bits; the LZ77 histogram
starts at word 34 and the 33-bin distance histogram starts at word 67. The WGSL `append_event` function derives the group's capacity from its checked
pixel count (`pixels + ceil(pixels / 8) + 1`), checks the event index before every record write,
and emits an overflow sentinel instead of indexing out of bounds. Host allocation reserves this
capacity independently for every group/channel, including per-device storage-offset alignment.
The header includes all three histograms even for ZeroRuns, whose explicit distance counts stay zero.
...
artifact. High-depth GPU tests also cover multiple batches, one-byte-deficient/exact budgets,
abandoned completion and reuse. The checked Gray8 fixture remains byte-for-byte stable.

Weighted encoding appends five words per column after each channel's maximum event range:
one signed true-error row and four unsigned subprediction-error words per column. Every
invocation owns its complete range and resets it before scanning; RLE compaction never skips
predictor updates. This uses the existing artifact binding, with no extra binding or submission.
The parameter ABI remains 256 bytes: predictor at byte 128, scratch word offset at 132,
coefficients at 136 and maximum weights at 164. LZ77 mode, scratch offset and hash mask occupy
bytes 180/184/188, followed by sixteen padding words from byte 192. Scratch offsets are relative
to the current artifact batch, just like event offsets. The added bytes participate in group
admission, batch splitting, direct-map/separate-readback accounting, pool leases and callback
lifetime. `weighted_predictor_scratch_bytes` reports the peak row-state subtotal already included
in `artifact_storage_bytes`; other predictors report zero. A complete component group is never
split across batches. The shared wide predictor temporaries remain private per invocation.

Greedy LZ77 appends two words per sample (packed residual and previous hash-chain link), followed
by one word per hash bucket. The bucket count is `min(next_power_of_two(pixels), 65536)`.
This scratch follows any Weighted row state in the same group/channel range. Each invocation
fills every residual, clears its bucket heads, then searches at most 32 candidates per token
position and inserts every consumed position, including those skipped by a match. No state is
shared between invocations or retained across groups/channels. Overlap is safe because every
source residual already exists. A general match uses two event records (length then distance)
for at least seven samples, so the existing event-capacity bound still suffices. The host
requires paired events, canonical extra bits, a positive distance within this channel's prior
samples and the 2²⁰ history limit, matching histograms and exact total sample coverage. Malformed
artifacts cannot become output packets. `lz77_scratch_bytes` is a peak subtotal already charged
in artifact/readback sizes; it adds no binding or submission. Streamed histogram and serialization
passes regenerate the same deterministic matches and retain the usual completion/cancellation
leases. The 400-byte header increases internal artifact storage, while default codestream bytes
remain unchanged.
...
Final packing uses either the shared 304-byte `ImageOutputParams` with planar source overrides or
...
The common packer selects one 304-byte parameter set for the actual original or linear-original
...
## Patch dictionary and pre-transform reference ABI

The patch frontend reads only entropy descriptors on the CPU. A single-invocation GPU parser
owns the ten-context dictionary, using the shared Prefix/ANS/hybrid/LZ77 machinery. Its first
pass validates and counts occurrences; the second allocates exactly `count * stride * 4` bytes
and writes resident commands. Empty dictionaries need no replay. Each command has eight geometry
words followed by three words for color and each extra: mode, alpha selector, clamp/association
flags. Geometry identifies the reference slot, source rectangle and destination position; its
reserved eighth word remains zero. Reference presence/domain, source/destination bounds, count
limits, signed offsets, blend modes and alpha selectors are validated before rendering.

The parser binds stream, entropy tables, continuation/history, commands and readonly parameters
at 0–4. Its 128-byte parameter storage contains a 12-byte entropy range, capacity at byte 12,
window geometry at 16, frame geometry at 32, limits at 48, and four reference records at 64.
Storage layout is intentional: nesting the 12-byte range in a uniform would change its ABI.
The first 32 scratch words retain parser and entropy state; an independently bounded LZ history
ring follows. At most 4096 parser steps run per submission, including zero-bit distributions.
Window continuation uses the common guarded overlapping upload ranges and preserves the cursor,
ANS state and pending LZ copy. Only four control words are mapped: error, bit cursor, parser
state and occurrence count. Replay must produce the same count and end cursor. Neither patch
values nor image samples cross the host boundary.

The renderer binds four references, commands, output and snapshot scratch at 0–6, with a
112-byte uniform at 7. Its canvas, command range and flags occupy three vectors; four reference
geometries follow at byte 48. One invocation owns every channel of one pixel and visits patch
occurrences in order. Each occurrence snapshots all channel results before publishing them,
including color source-over's selected-alpha update. Dispatches contain at most 64 occurrences;
an output-sized scratch allocation and all batch uniforms are charged before submission.
Inputs, output, commands and scratch remain owned by completion callbacks after cancellation.
...
The shared `ImageOutputParams` is 304 bytes: `alpha[0]` at byte 176 selects Preserve (0),
...
into the existing aligned all-channel render arena; its 304/160-byte uniforms and each 80-byte
Gaborish/EPF or 48-byte upsampling uniform are included exactly once. A 37×17, factor-2 color plan
with Gaborish and two EPF passes accounts 11,652 working-plane bytes and 928 uniform bytes,
...
an 80-byte normalization uniform and 464 bytes for packing. The tested 256-byte alignment makes
that total increase 158,488 bytes. One byte below the required capacity leaves no partial permit,
...
three 48-byte interpolation uniforms per level, and the color packer's 464-byte uniforms. The
...
## Analytic RGB profiles and original reconstruction

`RgbColorSpace::Custom(RgbChromaticities)` retains finite CIE xy primaries and the white point.
The shared host matrix validates source and target even for identity conversion, derives RGB/XYZ
and Bradford matrices in F64, and lowers once to three padded F32 rows. `WhitePointAdaptation::None`
preserves absolute XYZ. No image sample is processed on the CPU. Parameterized gamma values have
finite positive exponents and inverses; the parameter travels with its transfer variant.
Primary columns use homogeneous XYZ coordinates, so a zero-y primary is valid when the complete
...
`ImageOutputParams` is 304 bytes; `transfer_parameters` begins at byte 192. Shared image selectors
...
General CMS Gamma clamps negative values; general DCI retains a unit-slope negative branch.
JPEG XL original Gamma/DCI reconstruction additionally zeros target-linear values at or below
`1e-5` before the OETF, matching native libjxl's `OpGamma`. The same boundary is used for reference
storage, composition, LF presentation, native/scalar packing and original color output. It does
...
the 320-byte ICC dispatch storage, optional four-byte validation map, the 304-byte output-packing
...
`ImageOutputParams` is now 304 bytes. Its prior fields keep their offsets; `transfer_parameters.w`
...
## Requested tone-mapping coefficients

The 304-byte image-output record appends `ToneMappingParams` at byte 240 without moving earlier
fields. Its three vec4s store source/target white, the exclusive protected unit-luminance bound
and mode; PQ source minimum/span and normalized target endpoints; and knee/inverse width,
source-to-target white ratio and inverse PQ source span. Mode zero preserves the prior output.
Mapped output uses source intensity at input and target intensity at output. Codec source/output
uniforms total 464 bytes. ICC opcode 11 embeds the same 48-byte payload and a 16-byte stage entry;
its 320-byte dispatch and storage-binding count are unchanged. The mapping adds no image, dispatch
or readback. Exact admission tests exercise ordinary and tone-mapped output, including dynamic
black programs, shared program reuse, failed admission, reverse completion and cancellation.

## Requested RGB gamut mapping

`ImageOutputParams` is 304 bytes. Its final 16-byte `GamutMappingParams` vector begins at byte
288 and holds normalized target-primary luminances plus a saturation preference; a negative
preference disables mapping. The preceding 48-byte tone record stays at byte 240. Tone mode 5
preserves absolute light throughout when the protected threshold reaches either white, and
skips gamut mapping throughout. Other protected samples skip gamut mapping individually.

The shared final packer performs gamut mapping before transfer, alpha association, chroma
subsampling and quantization. Source/output uniforms total 464 bytes; ICC dispatch remains
320 bytes. There is no additional storage binding, image allocation or dispatch for the gamut
...
The gain kernel adds one 176-byte `Params` uniform: baseline offsets at byte 0 and strides at
16; three auxiliary `(offset, stride, width, height)` vectors at 32; minimum, maximum, inverse
gamma, baseline offset and alternate offset vectors at 80, 96, 112, 128 and 144. The 16-byte
application structure at 160 holds signed weight, baseline-to-reference-white scale, its inverse
and padding. All entries have 16-byte spacing, matching WGSL uniform layout. Naga validates the
portable shader and every member offset. The shared `ImageOutputParams` remains 304 bytes;
this submission reserves 480 uniform
...
before application. Their existing permits remain live while
