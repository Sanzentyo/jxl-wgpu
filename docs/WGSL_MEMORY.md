...
892 bytes and transform tasks remain 44 bytes; one DCT8 transform owns 12,304 bytes in total.
...
892 bytes, the main artifact header remains 272 bytes, and matrix/order entries remain 24 bytes.
...
They do not add GPU buffers, change the 892-byte parameters or 272-byte artifact header, or
