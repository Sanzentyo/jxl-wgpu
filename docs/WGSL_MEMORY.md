...
| `jxl_wgpu_encode/lossless_modular.wgsl` | `ModularParams` / `Params` | `width, height, row_stride, byte_offset, output_word_offset, channel, channels, bytes_per_sample, sample_mask`; 55 pads | 256 | 4 | read-only storage element |
...
| encoder `lossless_modular` | source words RO, artifact RW, 256-byte parameter records RO | 1x1 | Tier B (fixed) | one invocation per 256×256 PassGroup/channel; source subrange/alignment/u32 address and each group's artifact capacity are prevalidated; 1–31 valid bits use 1/2/4-byte native words |
