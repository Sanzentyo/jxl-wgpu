...
- Encoder artifact storage and mapped readback have identical checked, 4-byte-aligned sizes. The
  ...
  Mapped private scratch is not interpreted as pixels by the host: it reads only the count,
  validates `1..=k` and exact dynamic table coverage, then serializes encoded events. Event-count
