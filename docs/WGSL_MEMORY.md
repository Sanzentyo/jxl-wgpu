...
| `jxl_wgpu_encode/lossless_modular.wgsl` | `ModularParams` / `Params` | `width, height, output_word_offset, channel, channels, sample_mask, rct_type, big_endian`; four 24-byte source records (`row_stride, byte_offset, pixel_stride, word_bytes, bit_shift, plane`); predictor and WP scratch offset, seven coefficients, four maximum weights, 19 pads | 256 | 4 | read-only storage element |
...
- Encoder artifact storage and mapped readback have identical checked, 4-byte-aligned sizes. The
  ...
  Each channel's 268-byte header and 16-byte events are parsed as checked `Pod` records. Decoder codestream
...
Each Modular encoder group/channel artifact consists of 67 header words followed by four-word
events. The raw histogram has 33 bins, including token 32 with 31 extra bits; the LZ77 histogram
starts at word 34. The WGSL `append_event` function derives the group's capacity from its checked
...
The header adds 56 bytes per group/channel before alignment relative to the 19-bin raw alphabet.
...
coefficients at 136, maximum weights at 164 and padding from 180. Scratch offsets are relative
