...
- Codestream storage and the decoder prefix lookup are variable arrays, not fixed records. The
  ...
  directly from those spans. Neither engine constructs a second full-size host `Vec`. The public
  open path is still single-span. Incremental ingestion must also release consumed spans under the
  shared backpressure contract.
