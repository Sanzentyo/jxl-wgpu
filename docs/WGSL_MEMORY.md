...
| `vardct_image_upsample` (decoder, `upsample_rgb`) | resident X/Y/B and expanded weights RO, distinct presentation-resolution X/Y/B RW, U | 16x16 | Tier A (`KernelVariant` 2-D) | fused three-plane 2×/4×/8× frame upsampling using phase-major expanded 5×5 kernels; 48-byte uniform bounds input/output geometry and per-plane strides; checked storage limits and dispatch counts |
| decoder `lossless_gray8` | codestream/prefix RO, reconstructed/output/status RW, 244-byte parameter records RO, 16-byte dispatch U | 64x1 | Tier A (`KernelVariant` 1-D) | bounded `jwgp` index, aligned token words plus sentinel, per-group MA metadata base, channel-layout tables, four planes/final addresses, packed-row alignment, sample/output ranges and status allocation are prevalidated; one invocation per group lane; channel-fixed Gradient groups may resume through 16-byte-overlapped stream segments using one aligned 32-byte state record per lane |
...
values: token start/end bounds and the LZ77 ring mask. It begins the 244-byte Modular
