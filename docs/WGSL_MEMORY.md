...
- Codestream storage and the decoder prefix lookup are variable arrays, not fixed records. The
  ...
  and copier cross physical chunk boundaries; Modular also uses its span-native bit reader. VarDCT
  stream planning reads only the logical length, bounded windows copy exact span ranges, and its
  still-required whole-codestream GPU buffer is initialized while mapped directly from those spans,
  so neither engine constructs a second full-size host `Vec`. VarDCT's remaining metadata parsers
  and the public open path are still single-span. Incremental ingestion must also release consumed
  spans under the shared backpressure contract.
...
overlap/batching values without sharing their WGSL state layout. Each split segment is four-byte
