...
828 bytes and transform tasks remain 44 bytes; one DCT8 transform owns 12,240 bytes in total.
...
828 bytes, the main artifact header remains 272 bytes, and matrix/order entries remain 24 bytes.
...
`VarDctKernelParams` is an 828-byte storage record. Words 168–169 contain AC pass count/word
...
They do not add GPU buffers, change the 828-byte parameters or 272-byte artifact header, or
