...
| `vardct_packet` (decoder) | whole or reusable-window codestream/MA metadata RO, reconstruction/raw metadata/coefficients/status RW, control U, Modular params RW | 1x1 | Tier B (fixed) | combined global-tree and split LF/HF local-tree entry points use logical channel widths with explicit physical strides; staged local-tree LF entropy may resume across ordered windows using 64/128-byte aligned state, preserves LF storage, and maps only one bounded aggregate cursor/status set before whole-range HF |
...
AC and the staged local-tree VarDCT LF packet path use the same checked overlap/batching values
without sharing their WGSL state layout; VarDCT HF and combined/global packet adoption remains
unfinished. Each split segment is four-byte
...
group status into one staging map. Five words are reserved in the stable state layout for upcoming
combined LF/HF phase progress; they are not reused for diagnostics.
