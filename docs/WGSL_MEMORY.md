...
| `jxl_wgpu_encode/vardct_encoder/bounded.wgsl` | `VarDctKernelParams` / `Params` | source/block geometry, strategy/global/LF quantization, separate 19-entry DC and HF prefix tables, X/Y/B LF quantization, LF/HF X/B correlation, X/Y/B HF quantization, and explicit padding | 512 | 4 | read-only storage |
| `jxl_wgpu_encode/vardct_encoder/bounded.wgsl` | `VarDctKernelArtifact` / `Artifact` | 16-entry strategy map, 48 DC samples/tokens/extras, 64-word DC fragment, DC histogram, 256-word AC fragment, AC histogram, and 3×1024 forward/quantized XYB coefficient words | 26,880 | 4 | storage/readback record |
| `jxl_wgpu_encode/vardct_encoder/scalable.wgsl` | `ScalableVarDctKernelParams` / `Params` | source/block geometry, LF/control section ranges, topology/LF grid, LF quantization/correlation, separate 19-entry DC/HF prefix tables, HF correlation/quantization, five AC descriptor/fragment fields, 16 pads | 512 | 4 | read-only storage |
| `jxl_wgpu_encode/vardct_encoder/scalable.wgsl` | `ScalableVarDctArtifactHeader` / header words | status/live counts, AC-presence marker, LF section ranges/total bits, source/block geometry, topology, 19-bin DC histogram, LF descriptors/grid/count, AC descriptor offset/count and fragment offset/stride/word count, 13 pads | 256 | 4 | storage/readback record |
| `jxl_wgpu_encode/vardct_encoder/scalable.wgsl` | `ScalableDcFragmentDescriptor` / two words | `bit_offset, bit_len` for one row-major LF group | 8 | 4 | storage/readback element |
...
| `vardct_encoder/scalable::quantize_blocks` | source/params RO, artifact RW | 64x1 | Tier C (`KernelVariant` linear) | one 2-D workgroup per 8x8 block; checked block-grid/source/artifact ranges; 2,048 shared bytes; tiled DCT8 emits DC plus disjoint word-aligned AC fragments, other strategies emit DC |
| `vardct_encoder/scalable::serialize_control` | params RO, artifact RW | 1x1 | Tier B (fixed) | one bounded scalar dispatch serializes LF groups row-major, resets prediction at each 256x256-block boundary, and writes checked contiguous fragment descriptors |
...
| VarDCT encoder | `VarDctMemoryPlan` charges a 512-byte parameter, one artifact, and an equal-size readback. The bounded artifact is 26,880 bytes including diagnostics. Scalable LF sections have a 64-word header, two words per LF group, then independently 64-word-aligned strategy (`N`), DC/token/extra (`3N` each), and a checked entropy fragment. Tiled DCT8 adds `N` AC length words and `125N` fragment words, each section independently aligned. AC capacity is derived from actual prefix lengths for three counts and up to 63 coefficients per channel. The source binding is caller-owned. | The shared byte budget holds parameter + artifact + readback through validation or abandoned-job completion. Validators check every section field, DC residuals/histogram, AC count/coefficient bounds, exact bit consumption and zero padding. GPU workgroups own complete AC storage words; the host joins their bits in AC-group raster order without coefficient re-encoding. A 2-D block dispatch checks workgroup limits per axis; artifact binding and buffer limits independently cap the full allocation. | Non-DCT8 strategies remain zero-AC. Contextual/ANS policies cannot concatenate independent block fragments without maintaining their entropy state on GPU. Full 16K-square admission depends on adapter/budget capacity; actual-adapter cases include 2057x2057 nonzero AC and black 16Kx1/1x16K. Exact/one-byte-deficient budgets, nonzero abandoned-job reuse, artifact truncation/missing blocks/forged metadata and an insufficient device binding are tested. |
