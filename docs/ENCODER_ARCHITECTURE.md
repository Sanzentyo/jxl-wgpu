...
readback offsets and aggregate resource bounds. Each scalar `Plan` owns bounded rows and GPU
parameters. Dispatch and assembly consume this resolved topology rather than reinterpreting
metadata flags. Source windows are unioned by allocation identity for accurate alias accounting.
The configuration remains a lossless selection policy; numerical samples never enter host planning.

GPU row tasks compute raw-word Gradient residuals and prefix bits directly from the original
source windows, independently of ICC/color conversion. Only validated channel/row identity,
sample counts, exact entropy termination and zero padding establish artifact authority.
All planes share the color job's submission and map; its permit and completion owner retain
...
`SourceParams` (aliased as `ModularSourceParams`) / `Source` is 24 bytes and `ModularParams` / `Params` is 256 bytes, both
