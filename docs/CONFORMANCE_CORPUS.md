...
Compile-time assertions independently fix `ImageOutputUniform` at 304 bytes and its three padded
...
[`lossless_layouts::palette::mixed`](../crates/jxl_wgpu_encode/tests/lossless_layouts/palette/mixed.rs)
adds six GPU tests for dictionaries containing both absolute colors and predictor residuals.
The first distinct absolute tuples fill the requested color limit; later unknown tuples use
the separate delta dictionary. Existing absolute tuples always take priority. Prediction observes
every original post-RCT sample, including absolute selections. Equal words in different partitions
must remain distinct, and the GPU compacts reserved storage into used deltas followed by used colors
when producing table/index tokens. The host validates both counts before emitting their header.

Forty-two integer cases rotate all 42 RCT types, all 1–31-bit precisions, all 14 delta and entropy
predictors, custom Weighted coefficients, all group sizes, both trees and entropy policies, all
Squeeze choices and packed/planar/split addressing. Twenty-eight binary16/binary32 cases add all
delta predictors, Gray/GrayAlpha/RGBA, signed zero, NaN payloads and other special words. The
unchanged scalar libjxl word oracle checks every original bit; native F32 and whole/fragmented GPU
numeric output retain the same bounds and retained-output checks as the delta-only matrix above.

Eight Gray31 streams cover both count bucket families, including the simultaneous 70,911-color
and 66,816-delta maximum. Independent jxl-oxide frame/TOC and `jxl-bitstream` header readers verify
both used counts and the zero-delta edge group, without using jxl-oxide's delta inverse. Separate
header tests cross every color/delta bucket and predictor, including zero used deltas. A repeated
`7,14,7,21,7,28,7,14` source checks identical color/delta words and absolute reuse under all five
Squeeze choices. One absolute entry plus three West residuals succeeds; either the one-color or
three-delta policy alone rejects that source. This is a representability witness, not a measured
compression or runtime claim.

Four three-frame Replace animations and two cropped Add/Multiply sequences retain integer/IEEE
words, timing, source-layout changes, independent references and the existing native/Rust
composition bounds. Eight resident/streamed lifetime cases use Weighted delta and entropy
predictors, all group sizes, greedy LZ77, exact/one-byte-short admission, cancellation, source
retirement and pool reuse. Ten overflow cases, including failures after a valid streamed batch,
return no stream and release all resources before successful reuse. Constructor and artifact
tests reject invalid limits, truncated counts, total/delta overflow and either partition exceeding
its capacity. Existing color-only and delta-only case matrices remain unchanged.
...
input. Single/multiple groups, one-pixel axes, empty residual planes, selected-component
color/delta/mixed/implicit Palette and ordered RCT/Squeeze programs share complete group states.

jxl-oxide checks original integer/IEEE words and libjxl 0.12.0 checks original F32 output under the
unchanged `2e-7` integer and exact IEEE bounds. Delta/mixed/implicit Palette additionally requires
the pinned scalar native word oracle. Whole and 256-byte-window fragmented GPU output retain their
...
entropy decoding must consume exactly the fragment bits and finish at the normative ANS state.
A sentinel catches writes past output capacity; one-word output and absent symbols must fail.
Malformed completion records, missing events, truncation and nonzero tail bits cannot become packets.

The native word helper now handles the retained zero-symbol DC-global ANS state produced by
libjxl's `EncodeStream`/`WriteTokens`: its generic Modular decoder returns before reading that state
when no global samples exist. The helper invokes the unchanged native `ANSSymbolReader` and final
state check before its strict section-end check. It does not decode any image values itself or
ignore extra bytes. A regression corrupts both ends of the 32-bit state and unaligned padding;
both this oracle and whole/fragmented GPU decoding must reject every mutation without publishing
an image. No native entropy, inverse-transform or reference arithmetic is replaced.

One/multiple-batch ANS jobs check exactly two submissions per batch, compressed-storage accounting,
one-byte-short rejection before allocation, exact-budget completion, cancellation/source retirement
and buffer reuse. Late Palette/Squeeze overflow after valid batches returns typed errors with no
codestream and permits subsequent reuse. Tests run with two libtest threads on the actual adapter.
...
Modular/VarDCT sequence selection and [integer source precision](#integer-rgb-vardct-input) and [floating precision](#floating-rgb-vardct-input) have separate evidence; other VarDCT source colors, extra channels,
...
`vardct_encoder/tests/floating.rs`, `floating/boundaries.rs` and the shared
`tests/animation/sample_precision.rs` exercise all 154 legal floating precisions on
Apple M5/Metal (2026-09-25). The same checked sample/color plan binds canonical storage,
GPU conversion, bit-depth/exponent metadata and both mixed-codec profiles.

The 924-image matrix crosses every precision with XYB/original sRGB and single DCT8,
25×17 mixed maps and 259×3 tiled DCT8. Source words include signed zeros, subnormals and
normalized endpoints, with unaligned rows and poisoned padding. Single DCT8 additionally
requires clean/poisoned bytes to match, including standard `Float`/`CustomFloat` aliases.
Independent F64 arithmetic decoding and native impulse bases verify AC within one integer
quantizer step. Native/Rust/GPU F32 output retains `2e-4 * (1 + abs(reference))` and at most
one rounded RGB8 code; whole and 256-byte-window GPU results match exactly. Mixed/tiled
cases include five passes and saliency ordering. Source PSNR exceeds 30 dB with explicit
quantizers `(35252, 256, 48)` for this matrix.

Another 54 images cross all 27 strategies with binary16/XYB and binary32/original, custom
coefficient orders and LF metadata. Their stronger `(65536, 256, 256)` quantizers retain
30 dB source PSNR for these high-contrast patterns. The same AC bound remains; large-transform
F32 checks use the pinned scalar libjxl linear output, independently linearized Rust output
and whole/bounded GPU output at the unchanged `2e-4` relative-plus-absolute bound, with a
separate native-SIMD RGB8 comparison. This is measured corpus quality, not arbitrary-input
quality. A finite original-sRGB case additionally checks negative and greater-than-one input;
finite extreme input must produce the distinct typed quantization-overflow error.

A direct GPU conversion test compares 1,848 results (twelve values per precision) to F64
arithmetic converted to binary32: both signs of zero, minimum/maximum subnormal, minimum
normal, one and maximum finite. This checks conversion on the recorded adapter before lossy
transform arithmetic; it is not a portable promise to preserve subnormal bits through F32
arithmetic. Forty saliency cases cover four precisions, both paths and all five workgroup
variants against independent U64 edge/contrast sums of the clamped 8-bit proxy.

All 154 precisions reject positive/negative infinity and signed NaN payloads at first/last
pixels, rotating channels, through mapped XYB and tiled original RGB (1,232 jobs). Each
...
Sixteen fixed-codec sequences cross four precisions, both color domains, stills and animation.
Forty-eight mixed sequences cross those precisions, all three VarDCT backends, both timebases
and cropped/reference-only Add/Multiply versus full-canvas Replace controls. Native Modular
original words remain exact; native/GPU presentations match independent composition of
Rust-decoded physical stills. Reverse job insertion, five regular passes, indexed seeks,
retained output and complete budget release retain the existing bounds.

External whole-stream limitations remain explicit: jxl-oxide 0.13 disagrees on a 5-bit,
2-exponent-bit Modular-containing Replace stream (sample 0: 0.75 versus native/independent
0.5), and both oxide and Rust `jxl` disagree on mixed floating Add/Multiply (Rust sample 38:
1 versus native/independent 1.4981482). The cropped/reference cases retain native whole-stream,
GPU and independent Rust physical-frame composition checks; these external failures are not
counted as passes. The 24 added full-canvas Replace sequences use Rust `jxl` whole-stream
comparison. Pure VarDCT sequences retain jxl-oxide whole-stream comparison.

Reproduction: `cargo test --locked -p jxl_wgpu_encode --lib floating_precision -- --test-threads=2`,
...
extends the common contract to 1–31 bits; [floating input](#floating-rgb-vardct-input) adds all legal floating precisions. Automatic mode/quality selection and other color/alpha
...
cap. Exact layout metadata, shared 464-byte output/source uniform accounting, four-byte-rounded
...
304-byte uniform. Production still has no CPU image-domain fallback. Floating source samples remain
...
than the required uniform+ink metadata, then metadata admission fails with exactly 464 bytes for
five inks or 304 bytes for Preserve. Repeating failure rolls back output reservations; releasing
...
increase is 158,488 bytes. Original RGB VarDCT and 4:4:4/gray JPEG use the same 52,524-byte noise
...
## GPU splines and official animation precision (2026-09-13)

`tests/splines/main.rs` now executes the official `animation_spline` input through public
`GpuDecoder`, comparing all 60 frames of 320×320 RGB to the original float32 reference. The
reference is from `libjxl/conformance` commit
`b1d0f990b03e57bf6d137c365cd5dc8b470b9191`, with input identical to the workspace's existing
`fixtures/animation_spline.jxl`. `test-data/splines/animation_spline.npy.gz` preserves the original
NPY bytes with deterministic gzip compression; the test verifies SHA-256
`a571c5cbba58affeeb43c44c13f81e2b1962727eb9d4e017e4f25d95c7388f10`, little-endian F32 layout and
`(60, 320, 320, 3)` shape before comparing samples. The unchanged sRGB ICC and upstream license
are adjacent. Provenance and content-addressed download instructions are in that directory's
README. No CPU decoder generates the expected images during tests.

Every frame must meet its official maximum per-channel RMSE ≤ 0.0001 and absolute peak ≤ 0.004.
Values are compared without clipping negative or extended-range samples. Whole input and
fragmented input through 256-byte GPU windows must have identical output bits, frame indices,
durations and finality. A separate float32 `imqraw` run using
`imq compare - - --stdin-format imqraw --metrics rmse:color,maxae:color --format json` on each pair
measured maximum channel RMSE `3.698879245505896e-7` and peak `1.9118189811706543e-5` across the 60
320×320 RGB frames on Apple M5/Metal. Both records were RGBF32; no integer image loader or
quantization participates in these metrics. Reports and exact inputs are retained under
`.git/codex-validation/splines-20260913/imq-f32/`. This is one official case, not a complete ISO
18181-3 result or a performance claim.

The explicit `jxl_test_support::fixtures::splines` manifest adds 58 feature streams and four
progressive streams under `test-data/splines/`, regenerated by the `regenerate_splines` example.
Feature cases cover both coding modes, XYB/original sRGB, quantization adjustments -8/0/8,
changing signed coefficients and thickness, curved control points leaving/re-entering the image,
patches and reference overwrites, noise, standard/custom 2×/4×/8× upsampling, single/nested LF
producers and consumers, independent extras and 4:2:2/4:4:0/4:2:0 JPEG restoration. Native libjxl
must decode every generated input. Final references retain linear RGBA and every scalar extra.
Color uses normalized error ≤ 1/1024; alpha and numeric extras use ≤ 2e-6. Whole and fragmented
execution must agree bit-for-bit and release all reservations. These local tolerances do not
replace the official animation limits.

Reference selection is predetermined by source color and the manifest. XYB cases use native
linear output. Original-color cases use unconverted native sRGB followed by the signed analytic
f64 EOTF, avoiding the documented extended-range native CMS approximation. For example, native
sRGB 2.6743062 maps to 9.7885758 by that formula and the GPU returns 9.788584; native CMS instead
returns 9.752336. The `jpeg_440` and `jpeg_420` Gaborish/EPF3 cases use pinned jxl-oxide 0.12.6
linear output for the previously independently audited vertical-subsampling restoration defect.
Their native fast-renderer differences reach maxAE 0.175/0.209. Tolerances are not widened to
accept these reference defects.

Four two-frame patch/spline chains freeze native output for every progressive prefix and final
image, covering Modular/VarDCT, Gray/RGBA and mixed-precision extras. Independent selected-extra
requests follow the same prefix references. Single/nested LF updates additionally verify level
order, feature readiness, whole/bounded equality, final-only switching and held-image immutability.
The latter checks are lifecycle evidence, not independent LF pixel goldens. Private tests cover
Prefix, unary and LZ77 token runs across fields, exact cursors through unaligned 40-byte windows,
signed coefficient extremes, malformed counts/points, coordinate/delta limits, bounded geometry
work, exact draw/tile-reference allocation, empty caches and multi-dispatch ordered rendering.
Cancellation at entropy/geometry/raster boundaries, admission retry and terminal replay/body
allocation failure preserve previously committed references and drain the shared budget.
...
The complete serial regression passes on Rust 1.98.1 and Apple M5/Metal: 896 tests across 55
all-target/all-feature workspace targets, zero failures, no filtered tests and one existing ignored
allocation benchmark. All 884 previous test entries remain, including that ignored benchmark;
13 new tests cover spline parsing, geometry, rendering, feature/progressive combinations,
official animation precision and lifecycle limits. Five doctests pass. Formatting, workspace
check, Clippy and rustdoc with warnings denied, and six-library all-feature WebAssembly compilation
pass. Reference and Metal harnesses each pass 18/18. The Gray8 smoke test reads 221 bytes by direct
mapping with no staging allocation. All 1,082 patch/feature/seed/spline fixture files regenerate
identically. Source and fixture hashes confirm one unchanged implementation during the full
...
admission. Component previews omit both color conversion and its 464-byte uniform reservation.
...
[The gamut contract](GAMUT_MAPPING.md) and [native generator](../crates/jxl_wgpu/test-data/gamut_mapping_generator/README.md)
add 5,000 libjxl scalar records across BT.709/BT.2020/Display-P3 and five saturation preferences.
Actual Apple M5 Metal runs check 45,000 color components in three workgroup widths against
native and independent F64 intersections, with a predeclared `4e-6` linear allowance. They
check exact in-cube primitive values, alpha and output guards. Negative-luminance black,
extended values through ±1e30, subnormal signs and exact lower faces have separate tests.
Generic RGB8/F32, NV12 and 12-bit I444 output verifies mapping before transfer/packing; F32
encoded comparisons retain `5e-5`, and integer comparisons allow one code.

All 56 unchanged HDR sources / 80 images run with and without tone mapping, three output
transfers, both layouts and whole/bounded fragmented input: 1,920 presentations compare
3,362,688 color components against native reconstruction followed by independent F64 interval
conversion. Zero, relative, absolute and globally protected thresholds are covered. Eight
embedded ICC/XYB/Gray/RGB sources add 64 outputs / 29,376 color components. Held progressive
images remain immutable, final words agree, alpha words remain exact and budgets return to zero.
